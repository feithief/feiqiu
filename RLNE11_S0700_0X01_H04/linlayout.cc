...
bool validateLinLayout(const LinLayout &layout, QStringList *errors)
{
  ...
  if ((layout.colorModel < ELinColorModelPredefinedOnly) ||
      (layout.colorModel > ELinColorModelGenericSignals))
  ...
  if ((layout.colorModel != ELinColorModelGenericSignals) &&
      (layout.intensityMaximum == 0))
      ...
      (layout.publishedFrames != 0))
  {
         ...
         ++presetIndex)
    {
           ...
           ++assignmentIndex)
      {
        ...
        const bool validFrameIndex =
          (assignment.frameIndex >= 0) &&
          (assignment.frameIndex < layout.publishedFrameCount);
        const LinFrameLayout *presetFrame = validFrameIndex
          ? &layout.publishedFrames[assignment.frameIndex]
          : 0;
        const LinSignalLayout *matchedSignal = 0;
        if ((presetFrame != 0) && (presetFrame->signalLayouts != 0))
        {
          for (int signalIndex = 0;
               signalIndex < presetFrame->signalCount;
               ++signalIndex)
          {
            const LinSignalLayout &candidate =
              presetFrame->signalLayouts[signalIndex];
            if ((candidate.name != 0) && (assignment.signalName != 0) &&
                (std::strcmp(candidate.name, assignment.signalName) == 0) &&
                (candidate.startBit == assignment.startBit) &&
                (candidate.bitLength == assignment.bitLength))
            {
  ...
  const bool genericSignalModel =
    layout.colorModel == ELinColorModelGenericSignals;
  if (!genericSignalModel &&
      !primarySignalPresent[ELinSignalTargetMask])
    localErrors.append("Primary control frame has no target/address signal");
  if (!genericSignalModel &&
      !primarySignalPresent[ELinSignalIntensity])
  ...
  for (int index = 0; index < layout.nodeCount; ++index)
  {
    ...
    if (((layout.addressingModel != ELinAddressingModelImplicitPerSignal) &&
         (node.controlAddressMask == 0)) ||
        ((layout.addressingModel ==
                       ...
                       QString *errorMessage)
{
  ...
  {
    if (errorMessage != 0)
      *errorMessage = QString("Invalid signal preset");
    return false;
  }
  ...
  for (int index = 0; index < preset.assignmentCount; ++index)
  {
    ...
    {
      if (errorMessage != 0)
        *errorMessage = QString("Preset '%1' has an invalid frame index")
          .arg(QString::fromUtf8(preset.name));
      return false;
    }

    const LinFrameLayout &frame =
      layout.publishedFrames[assignment.frameIndex];
    const LinSignalLayout *matchedSignal = 0;
    for (int signalIndex = 0; signalIndex < frame.signalCount; ++signalIndex)
    {
      const LinSignalLayout &candidate = frame.signalLayouts[signalIndex];
      if ((candidate.name != 0) && (assignment.signalName != 0) &&
          (std::strcmp(candidate.name, assignment.signalName) == 0) &&
          (candidate.startBit == assignment.startBit) &&
          (candidate.bitLength == assignment.bitLength))
    ...
    {
      if (errorMessage != 0)
        *errorMessage = QString("Preset '%1' signal '%2' is not in frame %3")
          .arg(QString::fromUtf8(preset.name))
          .arg(QString::fromLatin1(assignment.signalName))
          .arg(QString::fromLatin1(frame.name));
      return false;
    }
    assignLogicalSignal(values, *matchedSignal, assignment.value);
  }
