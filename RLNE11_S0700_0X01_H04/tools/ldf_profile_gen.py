...
COLOR_MODEL_ENUMS = {
    ...
    "generic_signals": "ELinColorModelGenericSignals",
}
...
ADDRESSING_MODEL_ENUMS = {
    ...
    "implicit_per_signal": "ELinAddressingModelImplicitPerSignal",
}
...
def build_profile(network: LdfNetwork, overlay: Dict[str, Any], overlay_sha256: str) -> Dict[str, Any]:
    ...
    intensity_maximum = config_int(
        profile_cfg.get("intensity_maximum", 0),
        "profile.intensity_maximum", 0, 0xFFFFFFFF,
    )
    dimming_maximum = config_int(
        profile_cfg.get("dimming_maximum", 0),
        "profile.dimming_maximum", 0, 0xFFFFFFFF,
    )
    ...
    if color_model in ("direct_rgb_only", "generic_signals") and palette:
        raise ProfileError(
            "the selected color model must use an empty color.palette"
        )
    if color_model != "generic_signals" and intensity_maximum == 0:
        raise ProfileError("profile.intensity_maximum must be positive for a color-specific model")
    if dimming_model != "none" and dimming_maximum == 0:
        raise ProfileError("profile.dimming_maximum must be positive when dimming is enabled")
    ...
    for index, raw_node_cfg in enumerate(node_cfgs):
        ...
        signature = tuple(
            (
                item["name"], item["enum"], item["start_bit"], item["bit_length"],
    ...
    # Optional application defaults are keyed by exact LDF signal name and
    # work for every model (RGB, U/V, CCT, mode bits, future vendor fields).
    signal_defaults = config_object(
        overlay.get("signal_defaults", {}), "signal_defaults"
    )
    for signal_name, raw_value in signal_defaults.items():
        if not isinstance(signal_name, str) or not signal_name:
            raise ProfileError("signal_defaults keys must be exact signal names")
        matches = [
            (frame_name, binding)
            for frame_name, bindings in bindings_by_frame.items()
            for binding in bindings
            if signal_name in binding.get("source_signals", [])
        ]
        if len(matches) != 1:
            raise ProfileError(
                "signal_defaults.{0} must match exactly one selected master signal; found {1}".format(
                    signal_name, len(matches)
                )
            )
        _, binding = matches[0]
        if binding.get("source_signals") != [signal_name]:
            raise ProfileError(
                "signal_defaults.{0} targets one part of a combined typed binding; "
                "set control_signals default instead".format(signal_name)
            )
        binding["default"] = config_int(
            raw_value,
            "signal_defaults." + signal_name,
            0,
            (1 << binding["bit_length"]) - 1,
        )
    ...
    for preset_index, raw_preset in enumerate(preset_items):
        ...
        for signal_name, raw_value in values.items():
            ...
            matches = []
            for frame_index, frame in enumerate(published_frames):
                for binding in frame["bindings"]:
                    if binding.get("source_signals") == [signal_name]:
                        matches.append((frame_index, binding))
            if len(matches) != 1:
                raise ProfileError(
                    "{0}.values.{1} must match exactly one selected master signal; found {2}".format(
                        preset_path, signal_name, len(matches)
                    )
                )
            frame_index, binding = matches[0]
            signal_width = binding["bit_length"]
            if signal_width <= 0 or signal_width > 32:
                raise ProfileError(
                    "{0}.values.{1} uses unsupported signal width {2}; presets support 1..32 bits".format(
                        preset_path, signal_name, signal_width
                    )
                )
            value = config_int(
                raw_value,
            ...
            assignments.append(
                {
                    ...
                    "bit_length": signal_width,
                    "value": value,
                }
            )
        assignments.sort(key=lambda item: (item["frame_index"], item["start_bit"]))
    ...
    if color_model != "generic_signals":
        required_semantics.update(("target_mask", "intensity"))
    ...
    if ("intensity" in primary_bindings and intensity_maximum >
            ((1 << primary_bindings["intensity"]["bit_length"]) - 1)):
    ...
    # Color-specific convenience models retain their proven visible-on startup
    # policy. Generic profiles use exact signal_defaults instead of invented
    # RGB semantics and may name the fields that firmware requires non-zero.
    if color_model != "generic_signals":
        startup_target = 0
    ...
    required_nonzero = config_list(
        overlay.get("startup_required_nonzero_signals", []),
        "startup_required_nonzero_signals",
    )
    for signal_name in required_nonzero:
        if not isinstance(signal_name, str) or not signal_name:
            raise ProfileError(
                "startup_required_nonzero_signals must contain exact signal names"
            )
        matches = [
            binding
        ...
        if len(matches) != 1:
            raise ProfileError(
                "startup_required_nonzero_signals entry {0} must match exactly once".format(
                    signal_name
                )
            )
        if matches[0]["default"] == 0:
            raise ProfileError(
                "startup-required signal has a zero default: " + signal_name
            )
    ...
    security = {
        ...
        "lock_state_readback_delay_ms": 1000,
        "lock_state_readback_service": 0x22,
    }
    if security_enabled:
    ...
    return {
        ...
        "startup_required_nonzero_signals": required_nonzero,
...
def emit_source(profile: Mapping[str, Any]) -> str:
    ...
    for preset_index, preset in enumerate(profile["signal_presets"]):
        ...
        for assignment in preset["assignments"]:
            lines.append(
                "  {{{signal}, {frame}, {start}, {length}, {value}}},".format(
                    signal=cpp_string(assignment["signal"]),
                    frame=assignment["frame_index"],
...
def make_initial_overlay(network: LdfNetwork, ldf_path: Path) -> Dict[str, Any]:
    ...
    return {
        ...
        "models": {
            "color": "generic_signals",
            "addressing": "implicit_per_signal",
            "dimming": "none",
