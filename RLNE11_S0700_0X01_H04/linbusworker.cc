...
void LinBusWorker::requestPriorityControlTransmission()
{
  ...
  /* Replace the pending set with every editable published frame. Rapid UI
   * edits coalesce to the latest BCMSignal snapshot; LIN I/O stays Worker-only. */
  ...
  if (linLayout != 0)
  {
    for (int frameIndex = 0;
         ...
         ++frameIndex)
    {
      const LinFrameLayout &frame = linLayout->publishedFrames[frameIndex];
...
void LinBusWorker::processScheduleSlot()
{
  ...
  {
    const int frameIndex = priorityFrameQueue.isEmpty()
      ? -1
      : priorityFrameQueue.head();
    const int transmitSlot = publishedFrameScheduleSlot(frameIndex);
    if (transmitSlot >= 0)
    {
  ...
  if (prioritySlot)
  {
    if (!priorityFrameQueue.isEmpty())
      priorityFrameQueue.dequeue();
    priorityControlPending = !priorityFrameQueue.isEmpty();
    normalSlotRequiredAfterPriority = !priorityControlPending;
  }
  else
  {
...
{
  return publishedFrameScheduleSlot(
    linLayout == 0 ? -1 : linLayout->primaryControlFrameIndex);
}

int LinBusWorker::publishedFrameScheduleSlot(int frameIndex) const
{
  if (linLayout == 0)
    return -1;
       ...
       ++slotIndex)
  {
    if (linLayout->schedule[slotIndex].type ==
        ELinScheduleTransmitFrame &&
        linLayout->schedule[slotIndex].tableIndex == frameIndex)
      return slotIndex;
  }
                                     ...
                                     const QByteArray &value)
{
  ...
  if (writeComplete &&
      temporaryNadActive &&
      (service.operation == EOperationTypeSingleAddr) &&
      (value.size() >= 2))
  {
    const quint16 changedNad =
      static_cast<quint8>(value.at(0)) |
