...
def prefer_readable_cc_sources(project_root: Path, pro_text: str) -> str:
    """Use a readable .cc twin when DLP protects the copied .cpp file."""
    cpp_names = sorted(set(re.findall(r"[A-Za-z0-9_./-]+\.cpp", pro_text)))
    for cpp_name in cpp_names:
        cpp_path = project_root / cpp_name
        cc_name = cpp_name[:-4] + ".cc"
        cc_path = project_root / cc_name
        if not cc_path.is_file():
            continue
        try:
            cc_path.read_text(encoding="utf-8-sig")
        except UnicodeDecodeError:
            continue
        try:
            cpp_path.read_text(encoding="utf-8-sig")
            cpp_readable = True
        except UnicodeDecodeError:
            cpp_readable = False
        if not cpp_readable:
            pro_text = re.sub(
                r"(?<![A-Za-z0-9_./-])" + re.escape(cpp_name) + r"(?![A-Za-z0-9_.-])",
                cc_name,
                pro_text,
            )
    return pro_text
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        pro_text = prefer_readable_cc_sources(stage, pro_text)
