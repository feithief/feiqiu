  ...
  colorPageSize(30)
{
  ...
  /* The mother Seed reserves these convenience controls, but a profile only
   * exposes them when the matching typed signal exists. Generic U/V, CCT or
   * vendor layouts use the dynamic all-signal page and leave no dead widgets. */
  const bool hasAddress =
    (layout.addressingModel != ELinAddressingModelImplicitPerSignal) &&
    primaryControlHasSignal(layout, ELinSignalTargetMask);
  ui->labelAddress->setVisible(hasAddress);
  ui->spinBoxAdress->setVisible(hasAddress);
  ui->horizontalSliderAdress->setVisible(hasAddress);
  ui->labelAddressMode->setVisible(hasAddress);
  ui->pushButtonSingle->setVisible(hasAddress);
  ui->pushButtonGroup->setVisible(hasAddress);

  const bool hasDimming = primaryControlHasSignal(
    layout, ELinSignalFadingTime);
  ui->labelDimRamp->setVisible(hasDimming);
  ui->spinBoxDimRamp->setVisible(hasDimming);
  ui->horizontalSliderDimRamp->setVisible(hasDimming);

  const bool hasIntensity = primaryControlHasSignal(
    layout, ELinSignalIntensity);
  ui->labelIntensity->setVisible(hasIntensity);
  ui->spinBoxIntensity->setVisible(hasIntensity);
  ui->verticalSliderIntensity->setVisible(hasIntensity);
