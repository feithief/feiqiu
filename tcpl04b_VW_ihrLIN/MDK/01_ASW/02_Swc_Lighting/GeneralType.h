...
/* ????2??¡ê???¡¤?¡ã?12 bytes
* 16¦Ë¦Ì£¤??¨² 65535 * 60mA / 20mA = 786420 = 0xBFFF4
* ?21bit?¡ê RGB1263bit¡ê?1bit reserved?¡ê 1264bit 8???/
...
typedef struct SLinSignals
{
	...
	//uint8_t  BCM_RGB_Intensitaet : 7;   /** ???¨¤????????¡ê?¨¢?¨A??????¡ã?ntensity¨¤?¨¢?¦Ë*/
	uint16_t BCM_RGB_Intensitaet : 8; /** ???¨¤????????¡ê?¨¢?¨A??????¡ã?ntensity¨¤?¨¢?¦Ë?¡ê??¦Ë?¦Ì?¨¢??*/
    
	uint16_t BCM_RGB_Dimmrampe : 4; /**<Dimming ramp.*/
	uint16_t BCM_RGB_Normierung_Intensitaet : 1; /**<Reserved. ???¨¤????????¡ê?¨¢?¨A??????¡ã?ntensity¨¤?¨¢?¦Ë*/
...
typedef union USysState
{
	...
	struct SLIN
	{
		uint16_t NewCmd : 1;		/* ?o¦¢????o?????¦Ë */
		uint16_t TimeBaseUpdate : 1;	/* rampe?¡ébasis¡ê?speed??Øî¡ä£¤¡¤¡é */
		uint16_t IntensityUpdate : 1;	/* ¨¢¨¢??£??¡¤¡é */
	...
	struct SError
	{
		...
		uint16_t Flag : 3;		/* bit0:Battery, bit1:LEDs, bit2:Temperature, ????????3??? */
...
typedef struct USysState
{
	struct SLIN
	{
		uint8_t NewCmd : 1;		/* ?o¦¢????o?????¦Ë */
		uint8_t TimeBaseUpdate : 1;	/* rampe?¡ébasis¡ê?speed??Øî¡ä£¤¡¤¡é */
		uint8_t IntensityUpdate : 1;	/* ¨¢¨¢??£??¡¤¡é */
	...
	struct SError
	{
		uint8_t Flag;		/* bit0:Battery, bit1:LEDs, bit2:Temperature, ????????3??? */
	}Error;  //bit 6~8
	struct SCali
	{
...
/* ??¨°??offsetof¦Ì???¡ê?¦Ì??¦¢2??3t¦Ì£¤??¨²¡À???2???¡ê?¡À¡ê?¦Ì??o§Õ??¡¤?¦¶§³?¦Ì? */
#ifndef _offsetof_
