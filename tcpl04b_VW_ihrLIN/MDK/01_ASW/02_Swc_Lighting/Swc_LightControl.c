...
static void __systemModeShowroom(SColorParams *inputColor)
{
  ...
  {
    colorIndex = 0;
    tickRecord = IoHwAb_Time_Get() + CDimmingTimeSlicePeriod;
    newShowroomColor = btrue;
  }
  /*if should change to next color*/
  if (DTickAfter(IoHwAb_Time_Get(), tickRecord))
  {
    colorIndex++;
    if (colorIndex >= DMAX_SHOWROOM_MODE_COLOR_AMOUNT)
      colorIndex = 0;

    tickRecord += CDimmingTimeSlicePeriod;
    newShowroomColor = btrue;
  }
...
static void __doBlink(uint8_t mode, tick_t period, SColorParams *inputColor)
{
  ...
  if (oldMode != mode)
  {
    tickRecord = IoHwAb_Time_Get() + period;
...
{
  tick_t currentTick = IoHwAb_Time_Get();
  ...
  inputColor->ledTemprature = IoHwAb_Temperature_GetLed();
