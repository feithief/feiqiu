...
void ld_assign_frame_id_range(l_u8 NAD, l_u8 start_index, const l_u8* const PIDs)
{
   ...
   if (lin_tx_state == 0u)
   {
      ...
      for(loc_i = 0u; loc_i < 4u; loc_i++)
      {
          ...
          * Reason: Diagnostic frame can contain the whole array for PIDs (max. 4). */
         LIN_DIAG_SEND_FRAME.byte[loc_i + 4u] = (*ld_tmp_arr)[loc_i]; /*lint !e662, !e661*/
      }
      g_lin_config_status = LD_SERVICE_BUSY;
...
l_u8 ld_read_configuration (l_u8* const data,l_u8* const length)
{
   ...
   else
   {
      ...
      *data = g_lin_prod_id.NAD;
      for(loc_i = 0u; loc_i < LIN_NUMBER_OF_FRAMES; loc_i++)
      {
         /* Possible access of out-of-bounds pointer (1 beyond end of data) by operator '['
          * Reason: Buffer length is provided and checked prior to this loop. */
         (*l_api_tmp_arr)[loc_i + 1u] = g_lin_frame_ctrl[loc_i].frame.pid; /*lint !e662, !e661*/
      }
      ret_val = LD_READ_OK;
   }
   return ret_val;
...
l_u8 ld_set_configuration (const l_u8* const data,l_u8* const length)
{
   ...
   {
      g_lin_prod_id.NAD = *data;
      for(loc_i = 0u; loc_i < LIN_NUMBER_OF_FRAMES; loc_i++)
      {
         /* Possible access of out-of-bounds pointer (1 beyond end of data) by operator '['
          * Reason: Buffer length is provided and checked prior to this loop. */
         g_lin_frame_ctrl[loc_i].frame.pid = (*l_api_tmp_arr)[loc_i + 1u]; /*lint !e662, !e661*/
      }
      ret_val = LD_SET_OK;
   }
   return ret_val;
...
l_u8 l_sch_tick (void)
{
   static l_u16 currentDelaySchedule = 0u;
   static l_u16 g_activeScheduleIdx = 0u;
#ifndef LIN_EVENT_TRIGGERED_FRAME_ENABLE
   static l_u16 g_ScheduleIdx = 0u;
   static l_u8 g_ScheduleFrmIdx = 0u;
#endif /* #ifndef LIN_EVENT_TRIGGERED_FRAME_ENABLE */
   l_u8  retval = 0u;
   ...
   if(0u == g_lin_zero_schedule)
   {
      ...
      if((g_ScheduleIdx != g_nextScheduleIdx) || (g_nextScheduleFrmIdx != 0xFFu))
      {
         ...
         /* (MISRA-C:2004 MISRA Rule 13.7/R, e774) Boolean within 'if' always evaluates to False
         * Reason: depends on generated code*/
         if ((*scheduleList[g_activeScheduleIdx]) == 0) /*lint !e774*/
         {
      ...
      if (0u == g_lin_zero_schedule)
      {
         if(currentDelaySchedule == 0u)
         {
            ...
            currentDelaySchedule = (*scheduleList[g_activeScheduleIdx])[g_ScheduleFrmIdx].frame_delay;
            ...
            if (g_ScheduleIdx == l_resolver_tbl[g_lin_coll_tbl_entry].resolver_sched_id)
            {
               ...
               if ((0u != g_lin_coll_res_active) && (g_ScheduleFrmIdx == (schedTblSizeList[g_ScheduleIdx] - 1u)))
               ...
               else
               {
                  ...
                  {
                     /* First Frame in resolver table is sent now */
                     if (schedTblSizeList[g_ScheduleIdx] == 1u)
                     {
                        /* Only one entry in resolver schedule table, switch back immediately. */
                        l_sch_set(g_lin_coll_schedidx_save, g_lin_coll_frameidx_save);
                     }
                     else
                     {
                        /* Prepare to switch back when table is longer than 1 entry and wait until table is processed completely. */
            ...
            /* Reduce remaining delay with main_cyclic_looptime(ms) in 100¦Ìs */
            currentDelaySchedule -=(10u * (LIN_TASK_CYCLE_MS));
         }
         else /* no frame valid this time */
         {
            ...
            retval = 0u;
            if (currentDelaySchedule <= (10u * (LIN_TASK_CYCLE_MS)))
            {
               currentDelaySchedule = 0u;
...
void ld_task(void)
{
   ...
   ld_config_task();
#endif /* #if (!defined(LIN_USER_DEF_DIAG_ENABLE)) */
#endif /* #ifndef LIN_PROTOCOL_VERSION_1_3 */

#if (defined (LIN_USER_DEF_DIAG_ENABLE))
#if (defined (LIN_USER_DEF_DIAG_MANDSERV_ENABLE))
   /* Check if a request was received. The bit rx_config is only used for J2602 configurations. */
   if((g_lin_rx_ctrl.flag.rx_diag == 1u) || (g_lin_rx_ctrl.flag.rx_config == 1u))
   {
      ld_config_task();
...
#else /* (defined (LIN_USER_DEF_DIAG_MANDSERV_ENABLE)) */
   /* Check if a request was received. The bit rx_config is only used for J2602 configurations. */
   if((g_lin_rx_ctrl.flag.rx_diag == 1u) || (g_lin_rx_ctrl.flag.rx_config == 1u))
   {
...
#endif /* #if (defined (LIN_USER_DEF_DIAG_MANDSERV_ENABLE)) */
#endif /* #if (!defined(LIN_DIAG_ENABLE)) */
