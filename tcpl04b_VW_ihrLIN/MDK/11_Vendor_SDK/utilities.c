...
/********************************************************
** \brief   average_calculate_utils
**
** \param   int16_t*            data
** \param   uint16_t            length
**
** \retval  int16_t             averge
*********************************************************/
...
/********************************************************
** \brief   parity_calib_func
**
** \param   parity_type_e      type
** \param   uint16_t                pid
**
** \retval  None
*********************************************************/
...
{
    uint8_t ret;
    uint8_t parity = (((BIT32_VALUE_GET(pid, 0)^BIT32_VALUE_GET(pid, 1)^BIT32_VALUE_GET(pid, 2)^BIT32_VALUE_GET(pid, 4)) << 6) |
                      ((~(BIT32_VALUE_GET(pid, 1)^BIT32_VALUE_GET(pid, 3)^BIT32_VALUE_GET(pid, 4)^BIT32_VALUE_GET(pid, 5))) << 7));

    if (PARITY_CALIB_CHECK == type)
    {
        if ((pid & 0xC0) != parity)
        {
            ret = 0xFF;
