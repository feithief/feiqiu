                                     ...
                                     const QByteArray &value)
{
  ...
  int written = 0;
  quint8 finalConsecutiveData0 = 0xFF;
  quint8 finalConsecutiveData1 = 0xFF;
  bool finalConsecutiveEchoAvailable = false;
  ...
  if (success && (service.dataLength > 3))
  {
    ...
    while (success && (written < service.dataLength))
    {
      ...
      finalConsecutiveData0 =
        static_cast<quint8>(consecutiveFrame.at(2));
      finalConsecutiveData1 =
        static_cast<quint8>(consecutiveFrame.at(3));
      finalConsecutiveEchoAvailable = true;
  ...
  if (service.expectPositiveWriteResponse)
  {
    ...
    if (writeComplete)
    {
      const bool responseEnvelopeIsValid =
        (response.size() == 8) &&
        (static_cast<quint8>(response.at(0)) == requestNad) &&
        ((static_cast<quint8>(response.at(1)) & 0xF0) == 0x00) &&
        ((static_cast<quint8>(response.at(1)) & 0x0F) >= 3) &&
        (static_cast<quint8>(response.at(2)) == 0x6E);
      const bool didEchoIsValid =
        responseEnvelopeIsValid &&
        (static_cast<quint8>(response.at(3)) ==
         ((service.serviceId >> 8) & 0xFF)) &&
        (static_cast<quint8>(response.at(4)) ==
         (service.serviceId & 0xFF));
      const bool provenFinalFrameEchoIsValid =
        responseEnvelopeIsValid &&
        (linLayout->positiveWriteResponseEchoMode ==
         ELinPositiveWriteResponseEchoDidOrFinalConsecutiveData) &&
        finalConsecutiveEchoAvailable &&
        (static_cast<quint8>(response.at(3)) ==
         finalConsecutiveData0) &&
        (static_cast<quint8>(response.at(4)) ==
         finalConsecutiveData1);

      if (!didEchoIsValid && !provenFinalFrameEchoIsValid)
      {
        if ((response.size() == 8) &&
            (static_cast<quint8>(response.at(2)) == 0x7F) &&
      ...
      else if (provenFinalFrameEchoIsValid && (debug != 0))
      {
        debug->setValue(
          DebugDiagnosticState,
          QString::fromUtf8(
            "写入成功：已兼容从机多帧回显缺陷，DID=0x%1，应答=[%2]")
          .arg(static_cast<int>(service.serviceId),
               4, 16, QChar('0'))
          .arg(toHexText(response))
          .toUpper());
