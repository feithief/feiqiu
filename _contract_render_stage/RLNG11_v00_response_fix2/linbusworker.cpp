  ...
  if (service.expectPositiveWriteResponse)
  {
    ...
    if (writeComplete)
    {
      ...
      const bool didEchoIsValid =
        ...
        (static_cast<quint8>(response.at(4)) ==
         (service.serviceId & 0xFF));
      const bool profileAcceptsSidBeforeReadBack =
        responseEnvelopeIsValid &&
        (linLayout->positiveWriteResponseEchoMode ==
         ELinPositiveWriteResponseAcceptSidThenReadBack);

      if (!didEchoIsValid && !profileAcceptsSidBeforeReadBack)
      {
        if ((response.size() == 8) &&
            (static_cast<quint8>(response.at(2)) == 0x7F) &&
      ...
      else if (profileAcceptsSidBeforeReadBack && (debug != 0))
      {
        debug->setValue(
          DebugDiagnosticState,
          QString::fromUtf8(
            "已收到 0x6E 写应答：DID=0x%1；最终结果等待 1 秒后用 0x22 回读确认，应答=[%2]")
          .arg(static_cast<int>(service.serviceId),
               4, 16, QChar('0'))
          .arg(toHexText(response))
          .toUpper());
