...
```json
{
  ...
  "protocol_contract": {
    ...
    "diagnostics": {
      ...
      "positive_write_response_quirk": null,
      "security": {
...
`positive_write_response_echo_mode` defaults to `did`. Use
`sid_only_then_readback` only for a source-confirmed multi-frame `0x2E`
response-builder defect. Treat `0x6E` only as an acknowledgement and use the
delayed `0x22` read-back as the final success criterion. Add a
`positive_write_response_quirk` object with
