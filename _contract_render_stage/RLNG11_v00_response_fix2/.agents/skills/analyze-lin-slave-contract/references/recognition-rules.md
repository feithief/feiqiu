...
`positive_write_response_echo_mode=sid_only_then_readback` with evidence from
both the transport call site and response builder. In this mode, `0x6E` is only
a consumed transport acknowledgement; finish every write, wait the confirmed
flash window once, and use `0x22` values as the final success criterion. Keep
the default mode `did`; never enable this compatibility from a trace alone or
for all projects.
