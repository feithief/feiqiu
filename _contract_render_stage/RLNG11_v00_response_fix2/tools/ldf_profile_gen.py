...
POSITIVE_WRITE_RESPONSE_ECHO_ENUMS = {
    "did": "ELinPositiveWriteResponseEchoDid",
...
def build_profile(network: LdfNetwork, overlay: Dict[str, Any], overlay_sha256: str) -> Dict[str, Any]:
    ...
    positive_write_response_echo_mode = config_string(
        diag_cfg.get("positive_write_response_echo_mode", "did"),
        "diagnostics.positive_write_response_echo_mode",
    )
    if positive_write_response_echo_mode not in POSITIVE_WRITE_RESPONSE_ECHO_ENUMS:
        raise ProfileError(
            "diagnostics.positive_write_response_echo_mode must be one of: "
            + ", ".join(sorted(POSITIVE_WRITE_RESPONSE_ECHO_ENUMS))
        )
    ...
    return {
        ...
        "diagnostics": {
            ...
            "positive_write_response_echo_mode":
                positive_write_response_echo_mode,
            "positive_write_response_echo_enum":
                POSITIVE_WRITE_RESPONSE_ECHO_ENUMS[
                    positive_write_response_echo_mode
                ],
            "services": services,
...
def emit_source(profile: Mapping[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "  {0},".format(
                profile["diagnostics"]["positive_write_response_echo_enum"]
            ),
            "  {0},".format(bulk_ptr),
            "  {0},".format(len(bulk)),
            "  {{{0}, {1}, {2}, {3}, {4}}},".format(
