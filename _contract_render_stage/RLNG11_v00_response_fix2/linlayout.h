...
/* Standard 0x2E positive responses echo the DID.  A profile may explicitly
 * accept SID 0x6E as transport acknowledgement only when the slave source
 * proves its DID echo is broken and a delayed 0x22 read-back is configured. */
enum LinPositiveWriteResponseEchoMode
{
  ELinPositiveWriteResponseEchoDid = 0,
  ELinPositiveWriteResponseAcceptSidThenReadBack
};
...
/* One quick-action value written directly to a signal in the primary
 * master-published frame.  It intentionally uses the LDF bit position rather
 * than a UI semantic so presets can combine arbitrary control signals. */
...
struct LinLayout
{
  ...
  int serviceCount;
  LinPositiveWriteResponseEchoMode positiveWriteResponseEchoMode;
