...
def validate(project_root: Path) -> None:
    ...
    if node_count > 0 and diagnostic_model != "none":
        ...
        if worker:
            ...
            if write_service and not re.search(
                r"false\s*&&\s*service\.expectPositiveWriteResponse",
                write_service.group(0),
            ):
                errors.append("0x2E write may still wait for a 0x6E response")
        if isinstance(diagnostics, dict):
            if diagnostics.get(
