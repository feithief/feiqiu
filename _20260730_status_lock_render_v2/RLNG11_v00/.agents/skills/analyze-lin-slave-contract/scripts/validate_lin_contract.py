...
def validate(contract: Dict[str, object], source_root: Path) -> None:
    ...
    if isinstance(diagnostics, dict) and diagnostics.get("enabled") is True:
        ...
        if isinstance(security, dict):
            ...
            if security.get("lock_state_readback_delay_ms") != 1000:
                errors.append(
                    "protocol_contract.diagnostics.security."
                    "lock_state_readback_delay_ms must be 1000"
                )
            if security.get("lock_state_readback_service") != 0x22:
                errors.append(
                    "protocol_contract.diagnostics.security."
                    "lock_state_readback_service must be 34"
                )
    ...
    if isinstance(host_profile, dict):
        ...
        if isinstance(host_diagnostics, dict):
            ...
            if isinstance(host_security, dict) and host_security.get("enabled") is True:
                ...
                if host_security.get("lock_state_readback_delay_ms") != 1000:
                    errors.append(
                        "host_profile.diagnostics.security."
                        "lock_state_readback_delay_ms must be 1000"
                    )
                if host_security.get("lock_state_readback_service") != 0x22:
                    errors.append(
                        "host_profile.diagnostics.security."
                        "lock_state_readback_service must be 34"
                    )
                ...
                if isinstance(protocol_security, dict) and (
                    host_security.get("lock_state_readback_delay_ms")
                    != protocol_security.get("lock_state_readback_delay_ms")
                    or host_security.get("lock_state_readback_service")
                    != protocol_security.get("lock_state_readback_service")
                ):
                    errors.append(
                        "host/profile Lock State read-back rules do not match"
                    )
        ...
        if isinstance(frames, list):
            for frame in frames:
                if not isinstance(frame, dict):
                    continue
                if frame.get("direction") != "slave_to_master":
                    continue
                signals = frame.get("signals")
                if not isinstance(signals, list):
                    continue
                for signal in signals:
                    if isinstance(signal, dict) and isinstance(
                        signal.get("signal"), str
                    ):
                        feedback_names.add(signal["signal"])
        status_names = set()
        status_layouts = host_profile.get("status_layouts")
        if isinstance(status_layouts, list):
            for layout in status_layouts:
                ...
                if not isinstance(fields, list):
                    continue
                for field in fields:
                    if isinstance(field, dict) and isinstance(
                        field.get("name"), str
                    ):
                        status_names.add(field["name"])
        missing_feedback = feedback_names - status_names
        if missing_feedback:
            errors.append(
                "host_profile.status_layouts omit active feedback signals: "
                + ", ".join(sorted(missing_feedback))
            )

        if host_profile.get("startup_output_policy") != "visible_on_power_up":
            errors.append(
                "host_profile.startup_output_policy must be visible_on_power_up"
            )
        published = host_profile.get("published_frames")
        primary_index = host_profile.get("primary_control_frame_index")
        if (
            isinstance(published, list)
            and isinstance(primary_index, int)
            and 0 <= primary_index < len(published)
        ):
            primary = published[primary_index]
            bindings = {
                item.get("semantic"): item
                for item in primary.get("bindings", [])
                if isinstance(item, dict)
            }
            defaults = {
                semantic: item.get("default", 0)
                for semantic, item in bindings.items()
            }
            nodes = host_profile.get("nodes")
            models = host_profile.get("models")
            addressing = models.get("addressing") if isinstance(models, dict) else None
            if isinstance(nodes, list) and nodes:
                if addressing == "fixed_target_mask":
                    expected_target = 0
                    for node in nodes:
                        if isinstance(node, dict) and isinstance(
                            node.get("control_mask"), int
                        ):
                            expected_target |= node["control_mask"]
                else:
                    expected_target = nodes[0].get("control_mask", 0)
                if defaults.get("target_mask") != expected_target:
                    errors.append(
                        "startup target must select all fixed-mask nodes "
                        "or the first value-addressed node"
                    )
            if defaults.get("intensity", 0) <= 0:
                errors.append("startup intensity must be non-zero")
            if "led_enable" in bindings and defaults.get("led_enable") != 1:
                errors.append("startup LED-enable signal must be one")
            color_model = models.get("color") if isinstance(models, dict) else None
            if color_model in ("direct_rgb_only", "selectable"):
                for semantic in ("red_or_predefined", "green", "blue"):
                    item = bindings.get(semantic)
                    if isinstance(item, dict):
                        full_scale = (1 << item["bit_length"]) - 1
                        if item.get("default") != full_scale:
                            errors.append(
                                "startup {0} must be full-scale white"
                                .format(semantic)
                            )
                if (
                    "direct_rgb_enable" in bindings
                    and defaults.get("direct_rgb_enable") != 1
                ):
                    errors.append("startup direct-RGB enable must be one")
