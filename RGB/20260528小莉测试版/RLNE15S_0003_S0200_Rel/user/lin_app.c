...
l_u16 g_u16_BusIdleAutoSleepTimeOutCounter;
l_u16 g_u16_BusIdleMsgSleepTimeOutCounter;
E_SLEEP_TYPE g_e_SleepType;
#ifdef J2602_SLAVE
...
#define AUTO_SLEEP_TIMEOUT	10000U
...
void LIN_Task(void)
{
//  g_u16_BusIdleTimeOutCounter += LIN_TASK_CYCLE_MS;
...
//	if (g_lin_active_pid == 0X3C && g_lin_diag_mreq.frame.NAD == 0xFF)
//	{
...
//#endif
//  }LIN_TEMP_BUFFER.FRAME.DATA bUFFER[0]
//	if ((g_lin_active_pid == 0X3C && lin_temp_buffer.frame.DataBuffer[0] == 0x00)) 
//		//&& (lin_temp_buffer.frame.DataBuffer[5] == 0xFF &&lin_temp_buffer.frame.DataBuffer[6] == 0xFF &&lin_temp_buffer.frame.DataBuffer[7] == 0xFF &&lin_temp_buffer.frame.DataBuffer[1] == 0xFF  &&lin_temp_buffer.frame.DataBuffer[2] == 0xFF  &&lin_temp_buffer.frame.DataBuffer[3] == 0xFF &&lin_temp_buffer.frame.DataBuffer[4] == 0xFF ))
//	{
//#ifndef DLIN_COMM_NO_SLEEP_WHITE_LIGHT
//    if (g_st_SysStatus.e_SleepFlag == E_SYSTEM_SLEEP_AWAKE)
//      g_st_SysStatus.e_SleepFlag = E_SYSTEM_SLEEP_PREPARE;
...
//      g_st_SysStatus.e_SleepFlag = E_SYSTEM_SLEEP_PREPARE;
		if (g_st_SysStatus.e_SleepFlag == E_SYSTEM_SLEEP_AWAKE)
			g_e_SleepType = E_SLEEP_TYPE_COMMAND;
#endif
	}
	else
	{
		...
		{
			g_u16_BusIdleAutoSleepTimeOutCounter = 0u;
		}
		else
		{
			/* After 4 seconds no communication or other disturb - go to sleep */
			if(g_u16_BusIdleAutoSleepTimeOutCounter >= AUTO_SLEEP_TIMEOUT)//650 == 3.2s
			{
				g_u16_BusIdleAutoSleepTimeOutCounter = AUTO_SLEEP_TIMEOUT; /* prevent overflow */
#ifndef DLIN_COMM_NO_SLEEP_WHITE_LIGHT
				if (g_st_SysStatus.e_SleepFlag == E_SYSTEM_SLEEP_AWAKE)
					g_st_SysStatus.e_SleepFlag = E_SYSTEM_SLEEP_PREPARE;
#endif
		...
		switch (config_state)
		{
			...
			case 1u:
				if (ld_Rx_State == LD_COMPLETED)
				{
					...
					{
						if (SystemLinDiag_HandleRecv(&MasterReq_data[0u], ((uint16_t)MasterReq_data[1u]<<8|(uint16_t)MasterReq_data[2u]), MasterReq_length) == 0)
						{
							resp = RespErrorNone;
						}
						else
					...
					else if (MasterReq_data[0u] == 0x22u)
					{
						...
						{
							ret = SystemLinDiag_HandleSend(&SlaveResp_data[0u],((uint16_t)MasterReq_data[1u]<<8|(uint16_t)MasterReq_data[2u]), (uint16_t*)&SlaveResp_length);
							SlaveResp_length += 3;
					...
					{
						if (SystemLinDiag_UnlockHandle(&MasterReq_data[1u], MasterReq_data[1u], &SlaveResp_data[0u], (uint16_t*)&SlaveResp_length) == 0)
						{
							config_state = 2u;
							resp = RespErrorNone;
						}
