...
void lin_slave_task_tx_data(void)
{
   ...
   if((g_j2602_status.flags.status_byte_transm == 1u) && LIN_STATUS_BYTE_TRANSM())
   {
      ...
      if (g_j2602_status.flags.errorstate != 0u)
      {
         ...
         {
            g_j2602_status.flags.errorstate &= 0x0Fu;
         }
         else
         {
            ...
            else
            {
               ...
               {
                  g_j2602_status.flags.errorstate &= 0x03u;
               }
               else
               {
                  if ((g_j2602_status.flags.errorstate & 0x02u))
                  {
                     g_j2602_status.flags.errorstate &= 0x01u;
                  }
                  else
                  {
                     if ((g_j2602_status.flags.errorstate & 0x01u))
                     {
                        g_j2602_status.flags.errorstate = 0x00u;
                     }
                  }
               }
            }
...
#endif /* end #ifndef GM_SPEC_ADAPTION */
#endif /* end #ifdef J2602_SLAVE */
#ifdef LIN_TRANSMITTED_FRAME_COMPLETE
               l_callback_TXcomplete (g_lin_active_pid);
...
void lin_slave_task_rx_pid(l_u8 rx_pid_uc)
{
...
#ifndef SUP2BAUDRATES_NOPIDCHECK
...
#ifdef SUP2BAUDRATES_FRAMECOUNT
   l_u8 lin_autobaud_framecount = SUP2BAUDRATES_FRAMECOUNT;
#else
   l_u8 lin_autobaud_framecount = SUP2BAUDRATES_NUMBEROFFRAMES;
#endif /* #ifdef SUP2BAUDRATES_FRAMECOUNT */
#endif /* #ifndef SUP2BAUDRATES_NOPIDCHECK */
   ...
   if (L_RESET == lin_autobaud_flags_g.baudrate_is_identified)
   {
      ...
      if (VALID_PID_LIST[(l_u8)(rx_pid_uc & 0x3Fu)] == rx_pid_uc)
      {
         ...
         if ((lin_autobaud_flags_g.low_baudrate_frames >= lin_autobaud_framecount)
          || (lin_autobaud_flags_g.high_baudrate_frames >= lin_autobaud_framecount))
         {
            /* Valid baudrate is identified and will be set until next reset. */
            lin_autobaud_flags_g.baudrate_is_identified = L_SET;
         }
         /* Store the new default values for the low baudrate if enough low baudrate
            frames have been received. */
         if (lin_autobaud_flags_g.low_baudrate_frames >= lin_autobaud_framecount)
         {
            l_hal_set_to_low_baudrate();
         }
      }
      else
      {
   ...
   switch(rx_pid_uc)
   {
      ...
      case(0x7Du):
         ...
         l_callback_PIDreceived (rx_pid_uc);
#endif /* #ifdef LIN_PID_RECOGNITION_CALLBACK */
         /* Set frame index to the invalid value used for diagnostic frames. */
         g_lin_frame_index = 0xFBu;
#ifdef LIN_COOKED_API
         ...
         if (0u == g_j2602_status.flags.sresp)
         {
            break;
         }
#endif /* end #ifdef J2602_SLAVE */
...
/*Songjm Add for frame control*/
            if (lin_get_frame_status(loc_i) == 0)
            {
               continue;
            }
/*add end*/
#ifdef LIN_CYCLIC_TASK_PERFORMANCE
            /* (MISRA-C:2004 17.4/R, e960) pointer arithmetic other than array indexing used
             * Reason: Problem with struct access for this derivative is a problem with calculation time. Therefore this kind of access is used. */
            if(*(pointer+L_CTRL_PID_OFFSET) == rx_pid_uc) /*lint !e960*/
#else
...
#endif /* #ifdef LIN_CYCLIC_TASK_PERFORMANCE */
            {
               ...
               /* Protocol mask = 0x80 */
               if((g_lin_frame_ctrl[loc_i].reg[3u] & LIN_PROTOCOL_MASK) > 0u)
#else
               if((g_lin_frame_ctrl[loc_i].reg[1u] & LIN_PROTOCOL_MASK) > 0u)
#endif /* end #if ((defined LIN_ENABLE_ASSIGN_FRAME_ID) || (defined LIN_ENABLE_RBI_RD_MSG_ID_PID) || (defined J2602_PROTOCOL)) */
               {
                  LinActiveProtocolType = LIN_2_X;
               }
               else
               {
                  LinActiveProtocolType = LIN_1_3;
               }
#ifndef J2602_SLAVE
#ifdef LIN_EVENT_TRIGGERED_FRAME_ENABLE
               /* event triggered frames */
               if(g_lin_frame_ctrl[loc_i].reg[L_CTRL_TYPE_OFFSET] & FRAME_IS_EVENT_TRIGGERED)
               {
                  ...
                  if(g_lin_frame_ctrl[frm_idx].reg[L_CTRL_TYPE_OFFSET] & PUBLISH)
                  {
                     if(g_lin_frame_ctrl[frm_idx].reg[L_CTRL_TYPE_OFFSET] & UPDATED_SIGNALS)
                     {
                        ...
                        lin_hal_tx_response(LinActiveProtocolType, l_LinData.Frame[frm_idx].frame_data, g_lin_frame_ctrl[frm_idx].reg[L_CTRL_LENGTH_OFFSET]);
                  ...
                  else
                  {
                     ...
                     lin_hal_rx_response(LinActiveProtocolType, g_lin_frame_ctrl[frm_idx].reg[L_CTRL_LENGTH_OFFSET]);
                     /* Keep information that current frame is an event triggered frame */
                     g_lin_rx_ctrl.flag.evttrgfrm = L_SET;
                  }
                  lin_frame_found = 1u;
...
#endif /* end #ifndef J2602_SLAVE */
               {
                  ...
                  if(g_lin_frame_ctrl[loc_i].reg[L_CTRL_TYPE_OFFSET] & PUBLISH)
                  {
                     ...
                     lin_hal_set_tx_pid(g_lin_active_pid);
                     lin_hal_tx_response(LinActiveProtocolType, l_LinData.Frame[loc_i].frame_data, g_lin_frame_ctrl[loc_i].reg[L_CTRL_LENGTH_OFFSET]);
                  }
                  else
                  {
                     /* config LIN for rx data */
                     lin_hal_set_rx_pid(g_lin_active_pid);
                     lin_hal_rx_response(LinActiveProtocolType, g_lin_frame_ctrl[loc_i].reg[L_CTRL_LENGTH_OFFSET]);
                  }
                  lin_frame_found = 1u;
                  break;
               }
            ...
            /* (MISRA-C:2004 17.4/R, e960) pointer arithmetic other than array indexing used
             * Reason: Problem with struct access for this derivative is a problem with calculation time. Therefore this kind of access is used. */
            pointer = pointer + L_CTRL_NEXT_OFFSET; /*lint !e960*/
#endif /* #ifdef LIN_CYCLIC_TASK_PERFORMANCE */
         }
         if (lin_frame_found == 0u)
         {
            ...
            lin_hal_wait_for_break();
               ...
               l_callback_PIDnotreceived (rx_pid_uc);
...
void lin_slave_task_rx_data(void)
{
   ...
   switch(g_lin_frame_index)
   {
      ...
      case(0xFA):
         ...
         if (g_lin_frame_data_size <= L_MAX_FRAME_LENGTH)
         {
      ...
      default:
         ...
         break;
               ...
               l_callback_RXcomplete (g_lin_active_pid);
