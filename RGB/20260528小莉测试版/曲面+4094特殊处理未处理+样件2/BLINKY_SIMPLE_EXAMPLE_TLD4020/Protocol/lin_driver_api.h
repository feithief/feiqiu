...
/**
   ...
   @par        Last frame PID is the PID last detected on the bus and processed in the node. If overrun is
...
#ifdef LIN_EXTERNAL_AUTOBAUD_TIMER
/**
   @brief      Callout to start the external timer for autobaud timeout
   @par        The user implementation of this function must start the external timer
   @param      void       No parameters
   @retval     void       No return values
*/
void l_autobaud_timer_start(void);

/**
   @brief      Callout to stop the external timer for autobaud timeout
   @par        The user implementation of this function must stop the external timer
   @param      void       No parameters
   @retval     void       No return values
*/
void l_autobaud_timer_stop(void);

/**
   @brief      Callback to notify the LIN driver about an expired autobaud timeout
   @par        The user implementation of this function call this function when the
               external timer has expired
   @param      void       No parameters
   @retval     void       No return values
*/
void l_autobaud_timer_expired(void);
#endif
...
#define ld_allow_response() do{ ld_put_raw(&LIN_DIAG_SEND_FRAME.frame.NAD); } while (0)
#endif /* end #ifdef LIN_RAW_API */
...
#define ld_allow_response() do{ g_lin_diag_config_active = 0x1u; \
                                         g_lin_tp_timeoutcounter = 0u; } while (0)
#else
#define ld_allow_response() do{ } while (0)
...
#define ld_allow_userdef_response() do{ g_lin_user_diag_active = 1u; } while (0)
#else
#define ld_allow_userdef_response() do{ } while (0)
...
/**
   ...
   @note    No additional handling required.
 */
#define l_ifc_t2()         do{lin_hal_T2_ISR();}while(0)
...
/**
   ...
   @par     Choice will be given by LDC-Tool if required.
 */
void GetTimeGetEtsDelayTime_ETFix (l_u16 ByteToTransmit);
...
/**
   ...
   @par     Choice will be given by LDC-Tool if required.
 */
#define Timer_ISR_ETFix(pc_Char)             (lin_hal_Timer_ISR(pc_Char))
...
/**
   ...
   @par     Choice will be given by LDC-Tool if required.
 */
#define GetTimerDelay_ETFix(x)               (lin_hal_Timer_Delay(x))
...
/**
   ...
   @par     This tasks provides the time base for the LIN driver and handles everything
            around diagnostic communication as well as recovery from error states.
 */
void ld_task(void);
...
/**
   ...
   @par        This function is called when the node has finished the PID recognition.
               ...
               by the node for use by the application or diagnostics.
   @see        LIN specification (LIN 1.3, 2.x)
   @pre        LIN driver initialized.
...
#ifdef LIN_PID_NOTRECOGNIZED_CALLBACK
/**
   @brief      Callback when PID is received and not recognized
   @par        This function is called when the node has finished the PID recognition.
               The function will provide the received PID only if the PID is not supported
   ...
   @retval     void                 No return value.
*/
void l_callback_PIDnotreceived (l_u8 receivedpid);
#endif /* #ifdef LIN_PID_NOTRECOGNIZED_CALLBACK */

#ifdef LIN_TRANSMITTED_FRAME_COMPLETE
/**
   @brief      Callback when frame is successfully transmitted
   @par        This function is called when the node has finished the frame transmission.
               The function will provide the PID only if the frame transmission was successful.
   @see        LIN specification (LIN 1.3, 2.x)
   @pre        LIN driver initialized.
   @param      framepid             Received PID from Master
   @retval     void                 No return value.
*/
void l_callback_TXcomplete (l_u8 framepid);
#endif /* #ifdef LIN_TRANSMITTED_FRAME_COMPLETE */

#ifdef LIN_RECEIVED_FRAME_COMPLETE
/**
   @brief      Callback when frame is successfully received
   @par        This function is called when the node has finished the frame reception.
               The function will provide the PID only if the frame reception was successful.
   @see        LIN specification (LIN 1.3, 2.x)
   @pre        LIN driver initialized.
   @param      framepid             Received PID from Master
   @retval     void                 No return value.
*/
void l_callback_RXcomplete (l_u8 framepid);
#endif /* #ifdef LIN_RECEIVED_FRAME_COMPLETE */

#ifdef SUPPORT_TWO_BAUDRATES
/* ---------------------------------------------------------------------------
 *  void l_hal_set_to_low_baudrate ()
 * --------------------------------------------------------------------------*/
/**
   @brief  Sets the LIN driver to synchronize only to the lower baudrate of the
           genLinConfig configuration
   @par    If the LIN driver supports autobaud and the driver shall remember the
           through a baudrate LIN driver initialization, the application shall check
           the set baudrate with the function l_hal_check_baudrate() before calling
           l_sys_init or a reset. Either use l_hal_set_to_low_baudrate or
           l_hal_set_to_high_baudrate to configure the needed baudrate.
   @param  void
   @retval void
 */
void l_hal_set_to_low_baudrate(void);

/* ---------------------------------------------------------------------------
 *  void l_hal_set_to_high_baudrate ()
 * --------------------------------------------------------------------------*/
/**
   @brief  Sets the LIN driver to synchronize only to the higher baudrate of the
           genLinConfig configuration
   @par    If the LIN driver supports autobaud and the driver shall remember the
           through a baudrate LIN driver initialization, the application shall check
           the set baudrate with the function l_hal_check_baudrate() before calling
   ...
   @retval void
 */
void l_hal_set_to_high_baudrate(void);

/* ---------------------------------------------------------------------------
 *  void l_hal_check_baudrate ()
 * --------------------------------------------------------------------------*/
/**
   @brief  Returns the detected baud rate
   @par    This function can be used to determine if the driver has already been set
           to one of the two configured baudrates.
   @param  void
   @retval 0 Baudrate not yet detected
   @retval all other values The baudrate which is currently set
 */
l_u16 l_hal_check_baudrate(void);
#endif /* #ifdef SUPPORT_TWO_BAUDRATES */
#if 0
#ifdef LIN_ENABLE_AA_BSM_B5
/**
   @brief      Callback when Auto Addressing (SNPD) is started
   @par        This function is called when the node receives an Auto Addressing Init request (0xB8 / 0xB5).
               The function must reset and disable all peripherals that have to be exlusively used
               for the respective Auto Addressing (SNPD) service, e.g. RGB LEDs, ADC,...
   @see        BSM_2008, ISO17987 specification
   @pre        LIN driver initialized.
*/
void l_callback_AA_assign_NAD_SNPD_start(void);

/**
   @brief      Callback when NAD shall be stored in NVM (requested by Auto Addressing (SNPD) service)
   @par        This function is called when the node receives an Auto Addressing Store NAD request (0xB8 / 0xB5).
               The function will provide the exit code and the new configured NAD.
               If this optional service is used, application shall store the given NAD in NVM.
   @see        BSM_2008, ISO17987 specification
   @pre        LIN driver initialized.
*/
...
/**
   @brief      Callback when Auto Addressing (SNPD) is finished
   @par        This function is called when the node receives an Auto Addressing finish request (0xB8 / 0xB5).
               The function will provide the exit code and the configured NAD.
   @see        BSM_2008, ISO17987 specification
   @pre        LIN driver initialized.
   @param      exitCode           Exit code 0u: addressed successfully, 1u: not addressed
   @param      NAD                Current NAD
*/
