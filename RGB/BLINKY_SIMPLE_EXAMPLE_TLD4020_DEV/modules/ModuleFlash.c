...
static void moduleFlashSetDefaultWhiteRgbMixParams(void)
{
    //80
    // savedConfig.whiteRgbMixParams.rgbPointX = 3624u;
    // savedConfig.whiteRgbMixParams.rgbPointY = 4579u;
    // savedConfig.whiteRgbMixParams.whiteRatio = 8135u;
    // savedConfig.whiteRgbMixParams.rgbRatio = 1865u;
    //90
    savedConfig.whiteRgbMixParams.rgbPointX = 2090u;
    savedConfig.whiteRgbMixParams.rgbPointY = 2176u;
    savedConfig.whiteRgbMixParams.whiteRatio = 9150u;
    savedConfig.whiteRgbMixParams.rgbRatio = 850u;
}
...
static bool_t moduleFlashWhiteRgbMixParamsInvalid(void)
{
    if ((savedConfig.whiteRgbMixParams.rgbPointX > 10000u) ||
        (savedConfig.whiteRgbMixParams.rgbPointY > 10000u) ||
        (savedConfig.whiteRgbMixParams.whiteRatio > 10000u) ||
        (savedConfig.whiteRgbMixParams.rgbRatio > 10000u))
    {
        return btrue;
    }

    if ((savedConfig.whiteRgbMixParams.rgbPointX == 0u) &&
        (savedConfig.whiteRgbMixParams.rgbPointY == 0u) &&
        (savedConfig.whiteRgbMixParams.whiteRatio == 0u) &&
        (savedConfig.whiteRgbMixParams.rgbRatio == 0u))
    {
        return btrue;
    }
...
void moduleFlashInit(void)
{
  ...
  if (savedConfig.magicNO != DMagicNumber) //+ whether crc is correct
  {
    ...
    //90
     ...
     savedConfig.redY = 25080;

    savedConfig.greenx = 1402;
    savedConfig.greeny = 7123;
    savedConfig.greenY = 43830;

    savedConfig.bluex = 1519;
    savedConfig.bluey = 289;
    savedConfig.blueY = 7792;

    savedConfig.whitex = 3330;
    savedConfig.whitey = 3480;
    savedConfig.whiteY = 48820;
    //70
...
//    savedConfig.blueY = 2300;
    ...
    moduleFlashSetDefaultWhiteRgbMixParams();
...
//  savedConfig.Vtemp = 94;
  ...
  if (moduleFlashWhiteRgbMixParamsInvalid() == btrue)
  {
    moduleFlashSetDefaultWhiteRgbMixParams();
...
//    for (i = 0; len != 0; i++, len-- ,src1++, src2++)
    {
    
//        if (*src1 != *src2) 
			  if (EFlashAutoAddressSaved == flashFlag) 
				{
					user_nvm_erase(DATA_FLASH_START, 0);
        ...
        {
            // 2��3y???����?a��?��3y2?��
                user_nvm_erase(DATA_FLASH_START, 1);
                ...
                user_nvm_write(DATA_FLASH_START+UC_FLASH_PAGE_SIZE, (uint8_t *)&savedConfig+UC_FLASH_PAGE_SIZE, UC_FLASH_PAGE_SIZE, NVM_OPTIONS_RETRY_MASK);
								user_nvm_write(DATA_FLASH_START+UC_FLASH_PAGE_SIZE*2, (uint8_t *)&savedConfig+UC_FLASH_PAGE_SIZE*2, UC_FLASH_PAGE_SIZE, NVM_OPTIONS_RETRY_MASK);
        }
				
				if ((flashFlag & (EFlashUpdateProcessing | EFlashAutoAddressSaved)) == 0)
				{
					user_nvm_erase(DATA_FLASH_START, 0);
          user_nvm_write(DATA_FLASH_START, (uint8_t *)&savedConfig, UC_FLASH_PAGE_SIZE, NVM_OPTIONS_RETRY_MASK);
				}
