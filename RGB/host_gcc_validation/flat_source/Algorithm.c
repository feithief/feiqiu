...
static fix16_t __algoGetRgbPointX(void)
{
    return __algoWhiteRgbParamToFix16(savedConfig.whiteRgbMixParams.rgbPointX);
}

static fix16_t __algoGetRgbPointY(void)
{
    return __algoWhiteRgbParamToFix16(savedConfig.whiteRgbMixParams.rgbPointY);
}

static fix16_t __algoGetWhiteRgbWhiteRatio(void)
{
    return __algoWhiteRgbParamToFix16(savedConfig.whiteRgbMixParams.whiteRatio);
}

static fix16_t __algoGetWhiteRgbPointRatio(void)
{
    return __algoWhiteRgbParamToFix16(savedConfig.whiteRgbMixParams.rgbRatio);
}
...
/*
 * Add this helper function inside Algorithm.c, before algoGetPWMOutput().
 *
 * Reason:
 *   The ratioK lookup is implemented in AlgoRatioK.c.
 *   The split calculation must stay in Algorithm.c because it uses Algorithm.c private
 *   static functions and variables:
 *     __algoWhiteRgbParamToFix16()
 *     __algoSplitTargetColorByWhite()
 *     __algoGetWhitePointX()
 *     __algoGetWhitePointY()
 *     __algoGetDutyRatio()
 *     __algoGetWhiteDutyRatio()
 */

static void __algoGetRatioKPwmOutput(SColorParams * const inputColor,
                                     SCIEColor * const targetColorPtr,
                                     SPWMParams *outputPWM,
                                     uint16_t ratioKTargetU,
                                     uint16_t ratioKTargetV)
{
    extern uint8_t AlgoRatioK_GetWhiteRatioQ10000(uint16_t targetU,
                                                  uint16_t targetV,
...
*@retval    always true.
*/
static void __algoApplyFixedWhiteRgbMix(const SCIEColor * const targetColorPtr)
{
    splitWhiteCIE.x = __algoGetWhitePointX();
    splitWhiteCIE.y = __algoGetWhitePointY();
    splitWhiteCIE.Y = fix16_mul(targetColorPtr->Y, __algoGetWhiteRgbWhiteRatio());

    splitPointCIE.x = __algoGetRgbPointX();
    splitPointCIE.y = __algoGetRgbPointY();
    splitPointCIE.Y = fix16_mul(targetColorPtr->Y, __algoGetWhiteRgbPointRatio());

    splitLambda = 0;
    splitOneMinusLambda = 0;
    splitRayT = 0;
    splitEdgeIndex = -2;
    splitTargetColorValid = btrue;
}
...
//	SCIEColor targetColor;
    ...
    outputPWM->PWM_B = 0;
    outputPWM->PWM_W = 0;
    // if (sysLin_Stack.RGB.BCM_RGB_Sonderfunktion == 1)
    // {
    //     inputColor->colorU = 200u;
    //     inputColor->colorV = 460u;
    // }
    /*get target CIE color by input LIN and other signals.*/
    ...
    if (sysLin_Stack.RGB.BCM_RGB_Sonderfunktion == 0)
    {
        ...
        // if (sysLin_Stack.RGB.BCM_RGB_Sonderfunktion == 1)
        {
            __algoApplyFixedWhiteRgbMix(&targetColor);
        }
        else
        {
