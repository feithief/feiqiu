...
static bool_t moduleFlashConfigIsValid(const SFlashContent *data)
{
    ...
    if ((data->platform > EPlatformTypeMLBevoVW) ||
        ...
        (data->whiteRgbMixParams.rgbPointX > 10000u) ||
        (data->whiteRgbMixParams.rgbPointY > 10000u) ||
        (data->whiteRgbMixParams.whiteRatio > 10000u) ||
        (data->whiteRgbMixParams.rgbRatio > 10000u))
    {
        return bfalse;
    }
...
static void moduleFlashLoad(SFlashContent  *data)
{
    ...
    {
        flashFlag &= (uint8_t)(~EFlashUpdateLocked);
    }
}
...
void moduleFlashInit(void)
{
  ...
  if (savedConfig.magicNO != DMagicNumber) //+ whether crc is correct
  {
    ...
    //90
     savedConfig.redx = 7027;
     savedConfig.redy = 2965;
     savedConfig.redY = 28940;

    savedConfig.greenx = 1372;
    savedConfig.greeny = 7064;
    savedConfig.greenY = 47430;

    savedConfig.bluex = 1520;
    savedConfig.bluey = 285;
    savedConfig.blueY = 8885;

    savedConfig.whitex = 3332;
    savedConfig.whitey = 3491;
    savedConfig.whiteY = 56220;
    //70
