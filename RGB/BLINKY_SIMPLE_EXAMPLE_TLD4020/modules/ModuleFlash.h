#ifndef __MOD_FLASH_H
#define __MOD_FLASH_H

#include <stdint.h>
#include "ModuleAdc.h"
#include "pmu.h"
#include "SystemType.h"
#include "flash.h"
/* ============================================================================ */
/* --------------------------- DEFINES AND MACROS ----------------------------- */
/* ============================================================================ */
/*This macro defines max length of serial Number*/
#define DMAX_SERIAL_NUMBER_LENGTH 20u

/*This macro defines max length of part NOumber*/
#define DMAX_PART_NUMBER_LENGTH   11u

/*Hardware section length.*/
#define DHARDWARE_VERSION_LENGTH  3U


#define btrue 1
#define bfalse 0
/* ============================================================================ */
/* -------------------- FORWARD DECLARATIONS / PROTOTYPES --------------------- */
/* ============================================================================ */
/*This ENUM indicates which platform we are using*/
typedef enum PlatformType
{
  EPlatformTypeMQB        = 0x0000, /**<MQB platform.*/
  EPlatformTypeMLBevo     = 0x0001, /**<MLBevo platform.*/
  EPlatformTypeMLBevoVW   = 0x0002, /**<MLBevoVW platform.*/
  EPlatformTypeMax        = 0xFFFF, /**<To make this ENUM's size equals two.*/
} EPlatformType;

/*@brief This structure contains flash memory.*/


typedef struct LineContent
{
  uint8_t buf[18];
}SLineContent;
  

/*This ENUM indicates flash operation's status.*/
typedef enum FlashUpdate
{
  EFlashUpdateFree       = 0x0,    /**<Flash is free for updating.*/
  EFlashUpdateProcessing = 0x1,    /**<Flash's memory updating is on process.*/
  EFlashAutoAddressStart = 0x2,    
	EFlashAutoAddressSaved = 0x4,
  EFlashUpdateLocked     = 0x80,   /**<Flash's memory has been locked.*/
} EFlashUpdate;
typedef enum HardwareType
{  
  EHardwareTypeNoCap = 0x11,
  EHardwareTypeCap   = 0x1A,
}EHardwareType;
/*@brief This structure contains flash memory.*/
typedef enum Addressreset
{
  Edefaultreset   = 0x0000,
  EStartreset  = 0x0001,
}EAddressreset;
typedef struct WhiteRgbMixParams
{
  uint16_t rgbPointX;    /**< RGB split point x, multiplied by 10000. */
  uint16_t rgbPointY;    /**< RGB split point y, multiplied by 10000. */
  uint16_t whiteRatio;   /**< White channel Y ratio, multiplied by 10000. */
  uint16_t rgbRatio;     /**< RGB split point Y ratio, multiplied by 10000. */
} SWhiteRgbMixParams;
typedef struct FlashContent
{
  uint16_t       crc;         /**<Flash content crc16 result.*/
  uint16_t       magicNO;     /**<magic no also using as version flag.*/
	EAddressreset  resetNO;     /**<resetNO also using as version flag,Used to reset three addresses.*/
  /*The old version this section has two bytes, we add two reserved bytes to prevent overflow.*/
  EPlatformType  platform;    /**<Platform information @EPlatformType.*/
  uint16_t       factor;   /**<this member contains this RGB module's group address.*/
	uint16_t       singleAddr;  /**<this member contains this RGB module's single address.*/
  uint8_t        pART;
	uint8_t        pDevelor;
	uint16_t       eolAddr;
	uint16_t       AutoAddr;
	uint16_t       writeAddr;
    
  uint16_t       redx;        /**<Current led red CIE x coordinate.*/
  uint16_t       redy;        /**<Current led red CIE y coordinate.*/
  uint32_t       redY;        /**<Current led red CIE Y coordinate.*/
//	uint16_t       a;
  uint16_t       greenx;      /**<Current led green CIE x coordinate.*/
  uint16_t       greeny;      /**<Current led green CIE y coordinate.*/
  uint32_t       greenY;      /**<Current led green CIE Y coordinate.*/
//	uint16_t       b;
  uint16_t       bluex;       /**<Current led blue CIE x coordinate.*/
  uint16_t       bluey;       /**<Current led blue CIE y coordinate.*/
  uint32_t       blueY;       /**<Current led blue CIE Y coordinate.*/
//	uint16_t       c;
	uint16_t       whitex;      /**<Current led white CIE x coordinate.*/
  uint16_t       whitey;      /**<Current led white CIE y coordinate.*/
  uint32_t       whiteY;
  
	uint8_t        shortRed;
	uint8_t        shortGreen;
	uint8_t        shortBule;
	uint8_t        openRed;
	uint8_t        openGreen;
	uint8_t        openBule;
  uint8_t        partNO[DMAX_PART_NUMBER_LENGTH+1];              /**<part number, add one byte for '\0'.*/
  uint8_t        hardwareVersion[DHARDWARE_VERSION_LENGTH+1];    /**<hardware version, add one byte for '\0'.*/
	
  EHardwareType  hardwareType;        /**<flag hardware type RLN211 or RLN211A.*/
	uint8_t        serialNO[DMAX_SERIAL_NUMBER_LENGTH];          /**<serial number, add one byte for '\0'.*/
  int16_t        RthermolRatio;       /**<Thermal ratio of red led.*/
  int16_t        GthermolRatio;       /**<Thermal ratio of green led.*/
  int16_t        BthermolRatio;       /**<Thermal ratio of blue led.*/
  int16_t        MCUthermolRatio;     /**<Thermal ratio of MCU.*/
  int8_t         RatingLowerVoltage;  /**<Compensation for low voltage.*/
  int8_t         RatingHigherVoltage; /**<Compensation for High voltage.*/
  uint8_t        RCapCompensate;      /**<Compensation for PWM lost causing by capacitor.*/
  uint8_t        GCapCompensate;      /**<Compensation for PWM lost causing by capacitor.*/
  uint8_t        BCapCompensate;      /**<TCompensation for PWM lost causing by capacitor.*/
	uint8_t        ProductID[5];        /**<supplier, function ,variant ID.*/
	uint8_t        CurrentNAD;
 uint16_t       ledSupplier; /**<led supplier using as lock functions and other special functions.*/
  
	
	uint8_t        brightness_factor;
      uint8_t        Dimmingtemp;
      uint8_t        Utemp;
      uint8_t        Vtemp;
	  
	  
	  uint8_t        error_ram;
	  uint8_t        error_rom;
	  uint8_t        error_NVM;
       uint8_t        x1;
       uint8_t        y1l;
       uint8_t        y1h;
       uint8_t        x2;
       uint8_t        y2l;
       uint8_t        y2h;
       uint8_t        x3;
       uint8_t        y3l;
       uint8_t        y3h;
       uint8_t        x4;
       uint8_t        y4l;
       uint8_t        y4h;
       uint8_t        x5;
       uint8_t        y5l;
       uint8_t        y5h;
       uint8_t        x6;
       uint8_t        y6l;
       uint8_t        y6h;
       uint8_t        x7;
       uint8_t        y7l;
       uint8_t        y7h;
       uint8_t        x8;
       uint8_t        y8l;
       uint8_t        y8h;
       uint8_t        x9;
       uint8_t        y9l;
       uint8_t        y9h;
			 uint8_t        u;
			 uint8_t        v;
			 uint8_t        U[10];
			 uint8_t        V[10];
       
       SWhiteRgbMixParams whiteRgbMixParams;
			 
//			 uint32_t				AutoSaveCnt;
//			 uint16_t       AutoDebug[64];
	
}SFlashContent;


  extern volatile EHardwareType moduleType;




/* ============================================================================ */
/* ----------------------------SYSTEM GLOBALE VARIABLES ----------------------- */
/* ============================================================================ */

/*******************************************************************************
*@brief This variant shows flash's current status.
*@Note It's dangerous to make this external, only for smaller code size.
********************************************************************************/
extern volatile EFlashUpdate flashFlag;

/*******************************************************************************
*@brief This variant shows current flash's content.
*@Note It's dangerous to make this external, only for smaller code size.
********************************************************************************/
extern SFlashContent savedConfig;
extern SLineContent savedProduct;
/*******************************************************************************
*@brief This variant indicates if slave single address has been updated.
*@warning Do not delete this flag, sometime we change slave address to 0xA0, 
    in order to know whether the flash need to be updated
*@Note It's dangerous to make this external, only for smaller code size.
********************************************************************************/
extern uint16_t lastSavedSingalAdress;





static void moduleFlashLoad(SFlashContent  *data);
bool_t __memcmp(uint8_t* src1, uint8_t* src2, uint16_t len);
bool_t GetEEPROMBusyState(void);
uint8_t flashTest(void);
void moduleFlashInit(void);
uint8_t flashByteRead(uint16_t AddrOffset);
void moduleFlashSave(void);
void moduleFlashProcess(void);

void MemoryEraseAndWrite(uint8_t* src1, uint8_t* src2, uint16_t len); 
bool_t moduleFlash1000TpRead(uint32_t offset, uint8_t *dst, uint32_t len);
status_t moduleFlash1000TpRequestWritePage(uint32_t pageIndex, const uint8_t *src);
status_t moduleFlash1000TpRequestErasePage(uint32_t pageIndex);





static const unsigned char aucCRCHi[] = {
	
};
static const unsigned char aucCRCLo[] = {
	
};




#endif                                                                  /* __MOD_FLASH_H                 */
