...
- Keep SecurityAccess manual-only. Ordinary read, write, calibration, and page
  entry must never send SID `0x27`. Put `Lock` and `Unlock` only below the
  diagnostic page's `Lock State`; remove every standalone unlock page/button.
  `Unlock` alone sends `0x27`, with the fixed mother-Seed rule
...
- Render status as compact two-column rows only: a shortened signal name on
  the left and its raw hexadecimal field value on the right. Do not render
  Normal/Error/Unknown translations.
