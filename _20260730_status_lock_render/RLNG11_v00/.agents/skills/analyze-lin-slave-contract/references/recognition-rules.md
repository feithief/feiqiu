...
- expose one manual `Unlock` command only, with four little-endian key bytes
  calculated as `seed + 0x0C04`;
- expose one manual `Lock` command only when writable DID `0x0002` has length
  two, sending data bytes `82 00`;
- keep both controls under `Lock State` and never generate a separate unlock
  screen.
...
or a valid diagnostic response for that same node. Display feedback fields as
short source signal names plus raw hexadecimal values; semantic error text is
debug interpretation, not the page value.
