...
function s = make_empty_result_struct()
    numericFields = { ...
        ...
        'Exact_Valid','Exact_Ra','Exact_Rmin8','Exact_RGBPoint_x','Exact_RGBPoint_y','Exact_RayT','Exact_SingleWhiteRatio','Exact_RGBWhiteRatio','Exact_R_weight','Exact_G_weight','Exact_B_weight', ...
        'Proj1_Valid','Proj1_Ra','Proj1_Rmin8','Proj1_SelectedGridTarget_x','Proj1_SelectedGridTarget_y','Proj1_StoredBestRgb_x','Proj1_StoredBestRgb_y','Proj1_RGBPoint_x','Proj1_RGBPoint_y','Proj1_RayT','Proj1_SingleWhiteRatio','Proj1_RGBWhiteRatio','Proj1_R_weight','Proj1_G_weight','Proj1_B_weight', ...
        'ProjK_Valid','ProjK_Ra','ProjK_Rmin8','ProjK_SelectedGridTarget_x','ProjK_SelectedGridTarget_y','ProjK_StoredBestRgb_x','ProjK_StoredBestRgb_y','ProjK_RGBPoint_x','ProjK_RGBPoint_y','ProjK_RayT','ProjK_SingleWhiteRatio','ProjK_RGBWhiteRatio','ProjK_R_weight','ProjK_G_weight','ProjK_B_weight','ProjK_KUsed', ...
        'Ratio1_Valid','Ratio1_Ra','Ratio1_Rmin8','Ratio1_SelectedGridTarget_x','Ratio1_SelectedGridTarget_y','Ratio1_RGBPoint_x','Ratio1_RGBPoint_y','Ratio1_RayT','Ratio1_SingleWhiteRatio','Ratio1_RGBWhiteRatio','Ratio1_R_weight','Ratio1_G_weight','Ratio1_B_weight','Ratio1_KUsed', ...
        'RatioK_Valid','RatioK_Ra','RatioK_Rmin8','RatioK_SelectedGridTarget_x','RatioK_SelectedGridTarget_y','RatioK_RGBPoint_x','RatioK_RGBPoint_y','RatioK_RayT','RatioK_SingleWhiteRatio','RatioK_RGBWhiteRatio','RatioK_R_weight','RatioK_G_weight','RatioK_B_weight','RatioK_KUsed', ...
        'PureRGB_Valid','PureRGB_Ra','PureRGB_Rmin8','PureRGB_RGBPoint_x','PureRGB_RGBPoint_y','PureRGB_RayT','PureRGB_SingleWhiteRatio','PureRGB_RGBWhiteRatio','PureRGB_R_weight','PureRGB_G_weight','PureRGB_B_weight', ...
        'Approx_Max_Ra','Overall_Max_Ra'};
    for i = 1:numel(numericFields)
        s.(numericFields{i}) = NaN;
    end
    s.Approx_Max_Source = "";
...
function item = fill_method_fields(item, prefix, result)
    item.([prefix '_Valid']) = double(result.Valid);
    item.([prefix '_Ra']) = result.Ra;
    item.([prefix '_Rmin8']) = result.Rmin8;

    if isfield(result, 'SelectedGridTargetXY')
        item.([prefix '_SelectedGridTarget_x']) = result.SelectedGridTargetXY(1);
        item.([prefix '_SelectedGridTarget_y']) = result.SelectedGridTargetXY(2);
    end
    if isfield(result, 'StoredBestRgbXY')
        item.([prefix '_StoredBestRgb_x']) = result.StoredBestRgbXY(1);
        item.([prefix '_StoredBestRgb_y']) = result.StoredBestRgbXY(2);
    end
    if isfield(result, 'RGBPoint_xyY')
        item.([prefix '_RGBPoint_x']) = result.RGBPoint_xyY(1);
        item.([prefix '_RGBPoint_y']) = result.RGBPoint_xyY(2);
    end
    if isfield(result, 'RayT')
        item.([prefix '_RayT']) = result.RayT;
    end
    if isfield(result, 'SingleWhiteRatio')
        item.([prefix '_SingleWhiteRatio']) = result.SingleWhiteRatio;
    end
    if isfield(result, 'RGBWhiteRatio')
        item.([prefix '_RGBWhiteRatio']) = result.RGBWhiteRatio;
    end
    if isfield(result, 'R_weight')
        item.([prefix '_R_weight']) = result.R_weight;
        item.([prefix '_G_weight']) = result.G_weight;
        item.([prefix '_B_weight']) = result.B_weight;
    end
    if isfield(result, 'KUsed') && isfield(item, [prefix '_KUsed'])
        item.([prefix '_KUsed']) = result.KUsed;
    end
end
