% CALCULATE_GUANGPU_METAMERIC_CRI_LATEST
% Minimal-change version based on calculate_guangpu_metameric_cri.m.
% Fixed-single-white + RGB ray scan for guangpu spectra.
% The mixed output is constrained to cfg.target_xyY.
...
% Backward compatible:
% If cfg.singleWhiteFile exists, use it as the fixed single-white spectrum.
% Otherwise keep the original behavior and read W.CSV as SingleWhite.
if isfield(caseCfg, 'singleWhiteFile') && ~isempty(caseCfg.singleWhiteFile)
    singleWhiteFile = fullfile(spectraDir, caseCfg.singleWhiteFile);
else
...
rayScanCount = get_optional_scalar(caseCfg, 'rayScanCount', 1001);
