% S14_VERIFY_SINGLE_COLOR_LOCAL_PLANE_WHITERATIO_INTERPOLATION
% Verify one target color by comparing:
%   1. Exact ray-scan best-Ra method
%   2. Local weighted regression WhiteRatio interpolation method
%
% S14 local continuous WhiteRatio method:
%   - Assume Best_SingleWhiteRatio is locally continuous
%   - Sort LUT points by distance to current target color
%   - Take nearest K valid grid points
%   - Fit a local weighted model: WhiteRatio = f(x, y) or f(u', v')
%   - Predict current target WhiteRatio from the local model
%   - Clamp it to the current target's valid white-ratio range
%   - Apply it to the current target ray and evaluate Ra
%
% Recommended when:
%   Dense-grid simulation supports that BestWhiteRatio is locally continuous.

clear;
clc;

%% User config
targetUvDiv5 = [57, 39];     % MCU-style input, u'v' in Q1000/5
...
fprintf('\n============================================================\n');
fprintf('S14 single-color verification: Exact vs local weighted WhiteRatio interpolation\n');
...
%% Stored-ratio method
fprintf('Running S14 local weighted WhiteRatio interpolation method...\n');
...
fprintf('\n================ S14 Local Weighted WhiteRatio Interpolation =================\n');
print_stored_ratio_summary(proj, targetUv, 'S14LocalFit');

if exact.Valid && proj.Valid
    fprintf('\n================ Delta =================\n');
    fprintf('Ra diff (Exact - S14LocalFit)         = %.6f\n', exact.Ra - proj.Ra);
    fprintf('WhiteRatio diff (Exact - S14LocalFit)= %.6f\n', ...
...
if ~isempty(proj.CandidateTable)
    ...
    fprintf('\nTop %d nearest points used for local WhiteRatio fit:\n', showCount);
