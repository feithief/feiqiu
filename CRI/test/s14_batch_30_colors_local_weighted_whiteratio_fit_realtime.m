% S14_BATCH_30_COLORS_LOCAL_WEIGHTED_WHITERATIO_FIT_REALTIME
% Batch calculate 30 MCU-style u'v' input colors in one project.
%
% Method:
%   1. Exact ray-scan best-Ra method
%   2. S14 local weighted WhiteRatio interpolation method
%
% Realtime output:
%   - Each color is calculated one by one.
%   - The result text window is updated immediately after each color finishes.
%   - CSV and TXT summaries are exported to triangle_grid_lut_output.
...
nearestGridK = 9;               % plane: >=3 recommended 8~12; quadratic: >=6 recommended >=12
nearestGridWeightPower = 2;     % local fit weight = 1 / distance^power
fitRcondThreshold = 1e-10;      % if local normal matrix is ill-conditioned, fallback to weighted average
rayScanCount = 1001;
pwmMax = 255;

fprintf('\n============================================================\n');
fprintf('S14 batch verification: Exact vs local weighted WhiteRatio interpolation\n');
fprintf('Color count          = %u\n', uint32(size(targetUvDiv5List, 1)));
fprintf('Nearest grid metric  = %s\n', nearestGridMetric);
fprintf('Local fit coordinate = %s\n', localFitCoordinate);
fprintf('Local fit model      = %s\n', localFitModel);
...
outputCsvFile = fullfile(outputDir, 's14_batch_30_colors_local_weighted_whiteratio_fit_results.csv');
outputTxtFile = fullfile(outputDir, 's14_batch_30_colors_local_weighted_whiteratio_fit_realtime_log.txt');
...
logLines = append_realtime_line(logLines, editHandle, '================ S14 Batch Realtime Result ================');
logLines = append_realtime_line(logLines, editHandle, sprintf('Color count = %u', uint32(size(targetUvDiv5List, 1))));
logLines = append_realtime_line(logLines, editHandle, sprintf('Y = %.6f, K = %u, metric = %s, fit = %s/%s', ...
    targetY_lm, uint32(nearestGridK), nearestGridMetric, localFitCoordinate, localFitModel));
...
logLines = append_realtime_line(logLines, editHandle, sprintf('%-4s %-5s %-5s %-9s %-9s %-10s %-10s %-10s %-10s %-10s %-7s %-12s %-18s', ...
    'No', 'U', 'V', 'ExactRa', 'S14Ra', 'RaDiff', 'ExactW', 'AvgW', 'FitW', 'UsedW', 'FB', 'S14RGBPWM', 'Status'));
logLines = append_realtime_line(logLines, editHandle, string(repmat('-', 1, 160)));
...
for idxColor = 1:n
    ...
    try
        ...
        rowOut.S14Valid = double(s14.Valid);
        ...
        if s14.Valid
            rowOut.S14Ra = s14.Ra;
            rowOut.S14UsedWhiteRatio = s14.SingleWhiteRatio;
            rowOut.S14DistanceWeightedAverageWhiteRatio = s14.RawWeightedWhiteRatio;
            rowOut.S14LocalFitPredictedWhiteRatio = s14.WeightedWhiteRatio;
            rowOut.S14RGBRatio = s14.RGBWhiteRatio;
            rowOut.S14OutputDeltaUv = s14.OutputDeltaUv;
            rowOut.S14LocalFitFallbackUsed = s14.LocalFitFallbackUsed;
            rowOut.S14LocalFitConditionValue = s14.LocalFitConditionValue;
            rowOut.S14RGBPoint_x = s14.RGBPoint_xyY(1);
            rowOut.S14RGBPoint_y = s14.RGBPoint_xyY(2);
            rowOut.S14RGBPoint_Y_lm = s14.RGBPoint_xyY(3);
            rowOut.S14_R_weight = s14.R_weight;
            rowOut.S14_G_weight = s14.G_weight;
            rowOut.S14_B_weight = s14.B_weight;
            rowOut.S14NearestLutIndex0 = s14.LutIndex0;
            rowOut.S14CandidateCount = s14.CandidateCount;
        end

        if exact.Valid && s14.Valid
            rowOut.RaDiff_ExactMinusS14 = exact.Ra - s14.Ra;
            rowOut.WhiteRatioDiff_ExactMinusS14 = exact.SingleWhiteRatio - s14.SingleWhiteRatio;
        end

        pwm = batch_calc_pwm([rowOut.S14_R_weight, rowOut.S14_G_weight, rowOut.S14_B_weight, rowOut.S14UsedWhiteRatio], pwmMax);
        rowText = sprintf('%-4u %-5u %-5u %-9.4f %-9.4f %-10.4f %-10.4f %-10.4f %-10.4f %-10.4f %-7u (%3u,%3u,%3u,%3u) %-18s', ...
            uint32(idxColor), uint32(rowOut.IntSig_u), uint32(rowOut.IntSig_v), ...
            rowOut.ExactRa, rowOut.S14Ra, rowOut.RaDiff_ExactMinusS14, ...
            rowOut.ExactWhiteRatio, rowOut.S14DistanceWeightedAverageWhiteRatio, ...
            rowOut.S14LocalFitPredictedWhiteRatio, rowOut.S14UsedWhiteRatio, ...
            uint32(rowOut.S14LocalFitFallbackUsed), pwm(1), pwm(2), pwm(3), pwm(4), 'OK');

        logLines = append_realtime_line(logLines, editHandle, rowText);
        fprintf('%s\n', rowText);
    catch ME
        rowOut.ErrorMessage = string(ME.message);
        rowText = sprintf('%-4u %-5u %-5u %-9s %-9s %-10s %-10s %-10s %-10s %-10s %-7s %-12s ERROR: %s', ...
            uint32(idxColor), uint32(rowOut.IntSig_u), uint32(rowOut.IntSig_v), ...
            '-', '-', '-', '-', '-', '-', '-', '-', '-', ME.message);
        logLines = append_realtime_line(logLines, editHandle, rowText);
        fprintf('%s\n', rowText);
...
if ishghandle(dialogHandle)
    set(dialogHandle, 'Name', 'S14 Batch Result - Done');
end
...
function row = make_batch_empty_row()
    ...
    row.ExactRa = NaN;
    row.S14Ra = NaN;
    row.RaDiff_ExactMinusS14 = NaN;

    row.ExactWhiteRatio = NaN;
    row.S14UsedWhiteRatio = NaN;
    row.S14DistanceWeightedAverageWhiteRatio = NaN;
    row.S14LocalFitPredictedWhiteRatio = NaN;
    row.WhiteRatioDiff_ExactMinusS14 = NaN;

    row.ExactRGBRatio = NaN;
    row.S14RGBRatio = NaN;
    row.S14OutputDeltaUv = NaN;
    row.S14LocalFitFallbackUsed = NaN;
    row.S14LocalFitConditionValue = NaN;

    row.ExactRGBPoint_x = NaN;
    row.ExactRGBPoint_y = NaN;
    row.ExactRGBPoint_Y_lm = NaN;
    row.S14RGBPoint_x = NaN;
    row.S14RGBPoint_y = NaN;
    row.S14RGBPoint_Y_lm = NaN;

    row.Exact_R_weight = NaN;
    row.Exact_G_weight = NaN;
    row.Exact_B_weight = NaN;
    row.S14_R_weight = NaN;
    row.S14_G_weight = NaN;
    row.S14_B_weight = NaN;

    row.S14NearestLutIndex0 = NaN;
    row.S14CandidateCount = NaN;
    row.ErrorMessage = "";
end
...
function [dialogHandle, editHandle] = create_realtime_result_window()
    ...
    try
        dialogHandle = dialog('Name', 'S14 Batch Result - Running', ...
                              'Position', [80, 60, 1380, 780], ...
                              'WindowStyle', 'normal');

        uicontrol('Parent', dialogHandle, ...
                  'Style', 'text', ...
                  'String', 'S14 batch result, one line is appended immediately after each color finishes.', ...
                  'Units', 'normalized', ...
