% CALCULATE_GUANGPU_TRIANGLE_GRID_LUT_OPTIMIZED_EXACT
...
triangleGridDivCount = 24;      % 24 equal divisions on each RGB triangle edge, gives (24+1)*(24+2)/2 = 325 points
rayScanCount = 1001;            % Candidate RGB points per target ray
exportCHeader = true;           % Export compact C LUT header
cHeaderFileName = 'guangpu_triangle_grid_best_rgb_uv_lut.h';
...
%% Step 7: build triangular target grid inside RGB gamut in CIE 1976 u'v'
targetGrid = make_triangle_target_grid_uv1976(actual_xyY.R(1:2), actual_xyY.G(1:2), actual_xyY.B(1:2), ...
    targetY_lm, triangleGridDivCount);

pointCount = height(targetGrid);
fprintf('\nTriangle grid divisions = %d, point count = %d\n', triangleGridDivCount, pointCount);
fprintf('Ray scan count per target = %d\n', rayScanCount);

if validateFastEvaluation
    validate_fast_cri_evaluation(targetGrid, singleWhiteXY, rgbTriangleXY, rgbSolveMatrix, ...
        spd, singleNorm, std, wl, dLambda, rayScanCount, fastPre, validationRaTolerance);
end

%% Step 8: scan every target grid point and find best-Ra RGB point
