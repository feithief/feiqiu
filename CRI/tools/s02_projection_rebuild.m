...
%    找最近有效网格点，取该网格点的 Best_RayT 长度比例，应用到当前目标色射线上，
%    不再做“最佳RGB点到当前射线的垂线投影”。
% 4. 纯RGB法：单白光不亮，RGB直接混到目标色，并参与最终 Ra 最大值比较。
% 5. 输出精确法/投影K点法/纯RGB法结果，包括 Ra、RGB点、RayT、RGB权重等。
%
% 使用前：
...
fprintf('\n============================================================\n');
fprintf('【强制重算版】S02 RGB最佳点投影法 + 纯RGB对比\n');
fprintf('输出字段：精确 / 投影1点 / 投影K点 / 纯RGB\n');
fprintf('如果仍看到“投影K点”，说明你运行的不是这个文件。\n');
fprintf('============================================================\n\n');
...
nearestGridMetric = 'uv1976';   % 固定按 CIE 1976 u''v'' 找最近点，更适合Ra查表
nearestGridK = 6;              % 最近K个网格点的最佳RGB点做加权投影，推荐6
nearestGridWeightPower = 2;    % 反距离权重幂次，推荐2
printRealtimeRa = true;       % true：每计算完一个目标色，立即输出一次Ra结果
...
for k = 1:nTarget
    ...
    try
        ...
        if printRealtimeRa
            fprintf('Ra结果 %02d/%02d: 精确法失败，无法输出有效Ra。\n', k, nTarget);
            drawnow;
        end
        allResult(k) = item;
        continue;
    ...
    if ~exact.Valid
        ...
        if printRealtimeRa
            fprintf('Ra结果 %02d/%02d: 精确法无有效解，无法输出有效Ra。\n', k, nTarget);
            drawnow;
        end
        allResult(k) = item;
        continue;
    ...
    % 纯RGB法：单白光不亮，RGB直接混到目标色。
    % 注意：精确射线扫描从 RayT=1 开始，理论上已包含纯RGB点；
    % 这里单独输出并参与最终最大值比较，方便检查和量产逻辑调用。
    pureRgb = calculate_pure_rgb_result(target_xyY, rgbSolveMatrix, spd, std, wl, dLambda);

    try
        grid1 = find_grid_rgb_projection_result(target_xyY, singleWhiteXY, rgbTriangleXY, rgbSolveMatrix, ...
            spd, singleNorm, std, wl, dLambda, lut, maxCandidateCheck, nearestGridMetric, ...
            1, nearestGridWeightPower);
    catch ME
        warning('目标点 %d 最近1点投影法失败：%s', k, ME.message);
        grid1 = empty_best_result();
    end

    try
        grid = find_grid_rgb_projection_result(target_xyY, singleWhiteXY, rgbTriangleXY, rgbSolveMatrix, ...
            spd, singleNorm, std, wl, dLambda, lut, maxCandidateCheck, nearestGridMetric, ...
            nearestGridK, nearestGridWeightPower);
    catch ME
        warning('目标点 %d 最近K点加权投影法失败：%s', k, ME.message);
    ...
    if pureRgb.Valid
        item.PureRGB_Valid = 1;
        item.PureRGB_Ra = pureRgb.Ra;
        item.PureRGB_Rmin8 = pureRgb.Rmin8;
        item.PureRGB_R_weight = pureRgb.R_weight;
        item.PureRGB_G_weight = pureRgb.G_weight;
        item.PureRGB_B_weight = pureRgb.B_weight;
        item.PureRGB_Is_ExactMax = double(abs(pureRgb.Ra - exact.Ra) <= 1e-4);
    else
        item.PureRGB_Valid = 0;
    end

    if grid1.Valid
        grid1StoredRgbUv = xy_to_uv1976(grid1.StoredBestRgbXY);
        grid1ProjectedRgbUv = xy_to_uv1976(grid1.RGBPoint_xyY(1:2));

        item.Grid1_Ra = grid1.Ra;
        item.Grid1_Rmin8 = grid1.Rmin8;
        item.Grid1_TargetToSelectedGrid_DeltaUV = grid1.SelectedGridDeltaUv;
        item.Grid1_SelectedGridRank = grid1.SelectedCandidateRank;
        item.Grid1_SelectedGridTarget_x = grid1.SelectedGridTargetXY(1);
        item.Grid1_SelectedGridTarget_y = grid1.SelectedGridTargetXY(2);
        item.Grid1_StoredBestRgb_x = grid1.StoredBestRgbXY(1);
        item.Grid1_StoredBestRgb_y = grid1.StoredBestRgbXY(2);
        item.Grid1_StoredBestRgb_u = grid1StoredRgbUv(1);
        item.Grid1_StoredBestRgb_v = grid1StoredRgbUv(2);
        item.Grid1_ProjectedRgb_x = grid1.RGBPoint_xyY(1);
        item.Grid1_ProjectedRgb_y = grid1.RGBPoint_xyY(2);
        item.Grid1_ProjectedRgb_u = grid1ProjectedRgbUv(1);
        item.Grid1_ProjectedRgb_v = grid1ProjectedRgbUv(2);
        item.Grid1_RawRayT = grid1.RawRayT;
        item.Grid1_RayT = grid1.RayT;
        item.Grid1_R_weight = grid1.R_weight;
        item.Grid1_G_weight = grid1.G_weight;
        item.Grid1_B_weight = grid1.B_weight;
        item.Grid1_SingleWhiteRatio = grid1.SingleWhiteRatio;
        item.Grid1_RGBWhiteRatio = grid1.RGBWhiteRatio;
        item.Grid1_KUsed = grid1.KUsed;
        item.Grid1_WeightPower = grid1.WeightPower;
        item.Grid1_NearestMode = grid1.NearestMode;
        item.ExactVsGrid1Rgb_DeltaUV = norm(exactRgbUv - grid1ProjectedRgbUv);
    end
    ...
    if grid.Valid
        ...
        item.Grid_KUsed = grid.KUsed;
        item.Grid_WeightPower = grid.WeightPower;
        item.Grid_NearestMode = grid.NearestMode;
    ...
    [item.Final_Max_Ra, item.Final_Max_Source] = select_final_max_ra( ...
        item.Exact_Ra, item.Grid1_Ra, item.Grid_Ra, item.PureRGB_Ra);

    if printRealtimeRa
        fprintf(['Ra结果 %02d/%02d: 精确=%.4f | 投影1点=%.4f | 投影K点=%.4f | 纯RGB=%.4f | ' ...
                 '最大=%.4f | 来源=%s\n'], ...
            k, nTarget, item.Exact_Ra, item.Grid1_Ra, item.Grid_Ra, item.PureRGB_Ra, ...
            item.Final_Max_Ra, char(item.Final_Max_Source));

        fprintf(['              目标xy=(%.4f, %.4f) | K参考网格xy=(%.5f, %.5f) | ' ...
                 'RayT=%.4f | W比例=%.4f | RGB比例=%.4f | K=%g\n'], ...
            item.Target_x, item.Target_y, ...
            item.SelectedGridTarget_x, item.SelectedGridTarget_y, ...
            item.Grid_RayT, item.Grid_SingleWhiteRatio, item.Grid_RGBWhiteRatio, item.Grid_KUsed);
...
function s = make_empty_result_struct()
    names = { ...
        ...
        'Grid_KUsed','Grid_WeightPower','Grid_NearestMode', ...
        'Grid1_Ra','Grid1_Rmin8','Grid1_TargetToSelectedGrid_DeltaUV','Grid1_SelectedGridRank', ...
        'Grid1_SelectedGridTarget_x','Grid1_SelectedGridTarget_y', ...
        'Grid1_StoredBestRgb_x','Grid1_StoredBestRgb_y','Grid1_StoredBestRgb_u','Grid1_StoredBestRgb_v', ...
        'Grid1_ProjectedRgb_x','Grid1_ProjectedRgb_y','Grid1_ProjectedRgb_u','Grid1_ProjectedRgb_v', ...
        'Grid1_RawRayT','Grid1_RayT','Grid1_R_weight','Grid1_G_weight','Grid1_B_weight', ...
        'Grid1_SingleWhiteRatio','Grid1_RGBWhiteRatio','Grid1_KUsed','Grid1_WeightPower','Grid1_NearestMode', ...
        'ExactVsGrid1Rgb_DeltaUV', ...
        'PureRGB_Valid','PureRGB_Ra','PureRGB_Rmin8','PureRGB_R_weight','PureRGB_G_weight','PureRGB_B_weight','PureRGB_Is_ExactMax', ...
        'Final_Max_Ra','Final_Max_Source', ...
...
function gridResult = find_grid_rgb_projection_result(target_xyY, singleWhiteXY, rgbTriangleXY, rgbSolveMatrix, ...
    spd, singleNorm, std, wl, dLambda, lut, maxCandidateCheck, nearestGridMetric, ...
    nearestGridK, nearestGridWeightPower)

    % 最近K点最佳RGB点加权投影法：
    % 1. 按 u'v' 或 xy 找最近候选网格点；
    % 2. 读取每个网格点离线精确扫描得到的 Best_RGBPoint；
    % 3. 只收集“投影到当前目标射线后仍有效”的候选点；
    % 4. 对这些候选的 Best_RGBPoint 做反距离加权，得到参考点 Pw；
    % 5. 将 Pw 正交投影到当前 W->T 射线，得到当前目标色实际使用的 RGB 点。
    targetXY = target_xyY(1:2);

    if strcmpi(nearestGridMetric, 'xy')
        lutTargetXY = [lut.Target_x, lut.Target_y];
        distanceList = vecnorm(lutTargetXY - targetXY, 2, 2);
        nearestMode = "xy";
    else
        targetUv = xy_to_uv1976(targetXY);
        lutTargetUv = [lut.Target_u1976, lut.Target_v1976];
        distanceList = vecnorm(lutTargetUv - targetUv, 2, 2);
        nearestMode = "uv1976";
    end
    ...
    validBestRgbXYList = nan(nearestGridK, 2);
    validTargetXYList = nan(nearestGridK, 2);
    validDistanceList = nan(nearestGridK, 1);

    validCount = 0;
    skippedBeforeValid = 0;
    firstValidResult = empty_best_result();
    ...
    for candidateRank = 1:candidateLimit
        ...
        if any(~isfinite(candidateBestRgbXY))
            skippedBeforeValid = skippedBeforeValid + 1;
            continue;
        end

        rawRayT = dot(candidateBestRgbXY - singleWhiteXY, rayDir) / rayNorm2;
        ...
        validBestRgbXYList(validCount, :) = candidateBestRgbXY;
        validTargetXYList(validCount, :) = [candidateRow.Target_x, candidateRow.Target_y];
        validDistanceList(validCount) = sortedDistanceList(candidateRank);

        if validCount == 1
            firstValidResult = candidateResult;
            firstValidResult.StoredBestRgbXY = candidateBestRgbXY;
            firstValidResult.SelectedGridDeltaUv = sortedDistanceList(candidateRank);
            firstValidResult.SelectedGridTargetXY = [candidateRow.Target_x, candidateRow.Target_y];
            firstValidResult.SelectedCandidateRank = candidateRank;
            firstValidResult.SkippedBeforeValid = skippedBeforeValid;
            firstValidResult.RawRayT = rawRayT;
            firstValidResult.KUsed = 1;
            firstValidResult.WeightPower = nearestGridWeightPower;
            firstValidResult.NearestMode = nearestMode;
        end
    ...
    validBestRgbXYList = validBestRgbXYList(1:validCount, :);
    validTargetXYList = validTargetXYList(1:validCount, :);
    validDistanceList = validDistanceList(1:validCount);

    zeroDistanceIndex = find(validDistanceList <= 1e-12, 1, 'first');
    if ~isempty(zeroDistanceIndex)
        weights = zeros(validCount, 1);
        weights(zeroDistanceIndex) = 1;
    else
        weights = 1 ./ (validDistanceList .^ nearestGridWeightPower);
        weights = weights ./ sum(weights);
    end

    weightedBestRgbXY = sum(validBestRgbXYList .* weights, 1);
    weightedRayT = dot(weightedBestRgbXY - singleWhiteXY, rayDir) / rayNorm2;

    if weightedRayT < 1 - 1e-10 || weightedRayT > boundaryRayT + 1e-10
        gridResult = firstValidResult;
        return;
    end

    weightedProjectedRgbXY = singleWhiteXY + weightedRayT .* rayDir;
    weightedResult = calculate_result_from_rgb_point(target_xyY, weightedProjectedRgbXY, weightedRayT, ...
        singleWhiteXY, rgbSolveMatrix, spd, singleNorm, std, wl, dLambda);

    if weightedResult.Valid
        gridResult = weightedResult;
        gridResult.StoredBestRgbXY = weightedBestRgbXY;
        gridResult.SelectedGridDeltaUv = validDistanceList(1);
        gridResult.SelectedGridTargetXY = validTargetXYList(1, :);
        gridResult.SelectedCandidateRank = 1;
        gridResult.SkippedBeforeValid = skippedBeforeValid;
        gridResult.RawRayT = weightedRayT;
        gridResult.KUsed = validCount;
        gridResult.WeightPower = nearestGridWeightPower;
        gridResult.NearestMode = nearestMode;
    else
        gridResult = firstValidResult;
    end
end
...
function [maxRa, sourceName] = select_final_max_ra(exactRa, grid1Ra, gridKRa, pureRa)
    values = [exactRa, grid1Ra, gridKRa, pureRa];
    names = ["ExactRayScan", "Grid1Projection", "GridKProjection", "PureRGB"];
