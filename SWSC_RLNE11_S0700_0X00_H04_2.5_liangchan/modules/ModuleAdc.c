...
//        }
    ...
    else if ( channel >= ADC_CH_LED0 && channel <= ADC_CH_LED2 )
    {
...
//  if(!LDRV->DIAG_VFWD0.bit.VAL_F)
    VRED = LDRV->DIAG_VFWD0.bit.VFWD;
}       
void BLUE_V(void)
{
//  if(!LDRV->DIAG_VFWD1.bit.VAL_F)
    VBLUE = LDRV->DIAG_VFWD1.bit.VFWD;
}  
void GREEN_V(void)
{
//  if(!LDRV->DIAG_VFWD2.bit.VAL_F)
    VGREEN = LDRV->DIAG_VFWD2.bit.VFWD;
}
void EOD(void)
{
  return;
