...
void normal_mode(void)
{
  ...
  //#if ((LDRV_MOD_CFG & LDRV_MOD_CFG_LD_ENABLE_Msk) == LDRV_MOD_CFG_LD_ENABLE_Msk)
    ...
    LDRV->PWM_ISET_CFG0.reg = (uint32) 0x28;
    LDRV->PWM_ISET_CFG1.reg = (uint32) 0x28;
    LDRV->PWM_ISET_CFG2.reg = (uint32) 0x28;
...
//	if((sysLin.BCM_RGB_Intensitaet == 0 && 
...
{
	static uint8 flag1 = 0, flag2 = 0, flag3 = 0, flag4 = 0, flag5 = 0, flag6 = 0;
  int16_t redVoltage = 2000, greenVoltage = 2500, blueVoltage = 2500;
  ELedStatus tmpRStatus=redStatus,tmpGStatus=greenStatus,tmpBStatus = blueStatus;
  
  
//    if (detectionMask == 0)       //delete ÎªÁË±àÒëÍ¨¹ý£¬ÔÝÊ±É¾³ý
//    {
//        return;
//    }
		redVoltage = (int16_t)(ReturnADCBuffer(ADC_CH_LED0) * 8000 / 2047);
		if (redStatus != ELedStatusNormal)
			FilteredRedVoltage = redVoltage;
		else
			FilteredRedVoltage = (FilteredRedVoltage*3 + redVoltage)/4;

		blueVoltage = (int16_t)(ReturnADCBuffer(ADC_CH_LED1) * 8000 / 2047);
		if (blueStatus != ELedStatusNormal)
			FilteredBlueVoltage = blueVoltage;
		else
			FilteredBlueVoltage = (FilteredBlueVoltage*3 + blueVoltage)/4;

		greenVoltage = (int16_t)(ReturnADCBuffer(ADC_CH_LED2) * 8000 / 2047);
  ...
  if ((tmpRStatus & (ELedStatusOpen|ELedStatusShort)) &&
      (statusRCounter > 0))
  {
	  statusRCounter--;
  ...
  if ((tmpGStatus & (ELedStatusOpen|ELedStatusShort)) &&
      (statusGCounter > 0))
  {
    statusGCounter--;
  ...
  if ((tmpBStatus & (ELedStatusOpen|ELedStatusShort)) &&
      (statusBCounter > 0))
    {
	  statusBCounter--;
...
//	if ((savedConfig.openRed == 1   ||
//			savedConfig.openGreen == 1  ||
//			savedConfig.openBule == 1   ||
//			savedConfig.shortRed == 1   ||
//			savedConfig.shortGreen == 1 ||
//			savedConfig.shortBule == 1 ) &&
//			((flashFlag & EFlashAutoAddressSaved) == 0))
//	{
//		flashFlag = EFlashAutoAddressSaved;
//		flag = 0;		
//	}
