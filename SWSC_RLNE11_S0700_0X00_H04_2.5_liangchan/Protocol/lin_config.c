...
#if (!defined(LIN_PROTOCOL_VERSION_1_3)) && ((!defined (LIN_USER_DEF_DIAG_ENABLE)) || (defined(LIN_USER_DEF_DIAG_MANDSERV_ENABLE)))
#if ((defined (LIN_COOKED_API)) || (defined (LIN_RAW_API)))
//								uint32_t debug_first_addr = 0xff;
//					
uint8_t AutoAdressFlg = 5;
uint8_t B0FLAG = 0;
...
#endif /* #if ((defined LIN_COOKED_API) || (defined LIN_RAW_API)) */
{
   ...
   switch(LIN_DIAG_RECEIVE_FRAME.frame.type.SF.SID)
   {
         ...
         if(((LIN_DIAG_RECEIVE_FRAME.frame.NAD == g_lin_prod_id.Initial_NAD)||(LIN_DIAG_RECEIVE_FRAME.frame.NAD == LIN_BROADCAST_NAD)) && (LIN_DIAG_RECEIVE_FRAME.frame.PCI == 0x06u))
         {
                ...
                ( LIN_DIAG_RECEIVE_FRAME.frame.type.SF.diag_data[3] == LIN_FUNCTION_ID_WILDCARD_HI ))))
            {
              ...
              g_lin_prod_id.NAD = LIN_DIAG_RECEIVE_FRAME.frame.type.SF.diag_data[4];
							if(g_lin_prod_id.NAD != 0XA0)
							{
								savedConfig.CurrentNAD = g_lin_prod_id.NAD;
								moduleFlashSave();
								B0FLAG = 1;
							}
							else
      ...
      case(0xB4u):
         ...
         break;
#endif /* #ifndef USER_DEFINED_B4_SERVICE */
      ...
      case(0xB5u):
         ...
         if(LIN_DIAG_RECEIVE_FRAME.frame.NAD == LIN_BROADCAST_NAD)
         {
            if ((LIN_DIAG_RECEIVE_FRAME.frame.PCI == 0x06u) && (LIN_DIAG_RECEIVE_FRAME.frame.type.SF.SID == 0xB5u)
							  &&((LIN_DIAG_RECEIVE_FRAME.frame.type.SF.diag_data[0] == LIN_SUPPLIER_ID_WILDCARD_LO) &&
                (LIN_DIAG_RECEIVE_FRAME.frame.type.SF.diag_data[1] == LIN_SUPPLIER_ID_WILDCARD_HI)))
            {
							extern uint8_t B0FLAG;
											 B0FLAG = 0;
							if (LIN_DIAG_RECEIVE_FRAME.frame.type.SF.diag_data[3] == 0x2)
							{
								extern uint8_t B0FLAG;
											 B0FLAG = 0;
							switch (LIN_DIAG_RECEIVE_FRAME.frame.type.SF.diag_data[2])
                     {
											 
											 static uint8_t AutoAddr_Step2_SuccessFlag = 0;
//												case D_AUTO_ADDR_STEP1_START_AA:
//												{
//													LIN_AutoAddr_Start();
//												}
//												break;
												case D_AUTO_ADDR_STEP2_ASSIGN_NAD:
												{
													
													AutoAdressFlg = 0;
														 if (LIN_AutoAddr_Assign_NAD() == AddressStatusSuccess)
														 {
																uint8_t new_add = lin_temp_buffer.frame.DataBuffer[7];
															  
																//save node;
																if ((new_add <1) || (new_add > 14))
																{
																	 new_add = 16;
																}
                           
//                           savedConfig.AutoAddr = (uint16_t)new_add;
//                           savedConfig.pART = Slave_Address_auto;
																systemLinCommChangeNad(new_add);
																assign_addr = new_add;
																AutoAddr_Step2_SuccessFlag = 1;
														 }
														 
//														 if (debug_first_addr == 0xff)
//														 {
//															 debug_first_addr = lin_temp_buffer.frame.DataBuffer[7];
//														 }
												}
												break;
                        case D_AUTO_ADDR_STEP3_STORE_NAD:
                     {
                        if (AutoAddr_Step2_SuccessFlag == 1)
                        {
                           savedConfig.AutoAddr = (uint16_t)assign_addr;
                           systemLinCommChangeNad(assign_addr);
                           flashFlag = EFlashAutoAddressSaved;
													 AutoAdressFlg = 0;
													 AutoAddr_Step2_SuccessFlag = 0;
                        }
                     }
                     break;
                        case D_AUTO_ADDR_STEP4_FINISH_AA:
                        {
                           LIN_AutoAddr_End();
													AutoAdressFlg = 5;
													LINAA_forceShutdown = btrue;
													forceReInitLin = btrue;
													AutoAdressFlgLin = btrue;
                        }
                        break;
                        default:
                        /*STEP1 AND STEP2 will be handled in other functions */
                        break;
                     }
									 }
            }
...
static void lin_handle_FunctionalNAD(void)
{
#if ((defined(LIN_ENABLE_ASSIGN_FRAME_ID_RANGE)) || (defined(LIN_ENABLE_ASSIGN_FRAME_ID)))
   l_u8 loc_i;
#endif /* end #if defined(LIN_ENABLE_ASSIGN_NAD) || defined(LIN_ENABLE_ASSIGN_FRAME_ID) || defined(LIN_ENABLE_ASSIGN_FRAME_ID_RANGE) */
#ifdef  LIN_ENABLE_COND_CHG_NAD
